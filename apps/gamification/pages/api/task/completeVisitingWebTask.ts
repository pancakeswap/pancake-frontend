...
const handler = async (req, res) => {
  if (!GAMIFICATION_PUBLIC_API || !req.query || req.method !== 'POST' || !!req?.headers?.authorization) {
    return res.status(400).json({ message: 'API URL Empty / Method wrong / Header Authorization Empty' })
  }
