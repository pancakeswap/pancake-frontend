...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      const { token, tokenSecret, twitterLikedId, providerId } = req.query
      if (!token || !tokenSecret || !twitterLikedId || !providerId) {
        res.status(400).json({ message: 'Missing required parameters: token, tokenSecret, twitterLikedId, providerId' })
        return
      }
      ...
      const url = `https://api.twitter.com/2/users/${twitterLikedId}/liked_tweets`
      const method = 'GET'
