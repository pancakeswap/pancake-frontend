...
const handler = async (req, res) => {
  if (!GAMIFICATION_PUBLIC_API || req.method !== 'POST' || !req?.headers?.authorization) {
    return res.status(400).json({ message: 'Invalid request method / Header Authorization Empty' })
  }
