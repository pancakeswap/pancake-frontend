...
export const EXPIRED_ORDERS_SWR_KEY = ['gelato', 'expiredOrders']
...
const useExpiredOrders = (turnOn: boolean): Order[] => {
  ...
  const { data } = useSWR(
    startFetch ? EXPIRED_ORDERS_SWR_KEY : null,
    async () => {
      try {
...
export default function useGelatoLimitOrdersHistoryData(orderCategory: ORDER_CATEGORY) {
