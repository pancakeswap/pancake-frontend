...
const useBoosterFarmHandlers = (farmPid: number, onDone) => {
  ...
  const activate = useCallback(async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(farmBoosterContract, 'activate', [BigInt(farmPid)], { gasLimit: BOOSTED_FARM_GAS_LIMIT })
    })
  ...
  const deactivate = useCallback(async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(farmBoosterContract, 'deactive', [BigInt(farmPid)], { gasLimit: BOOSTED_FARM_GAS_LIMIT })
    })
