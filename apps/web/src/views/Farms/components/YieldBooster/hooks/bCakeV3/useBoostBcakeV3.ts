...
export const useBoosterFarmV3Handlers = (tokenId: string, onDone: () => void) => {
  ...
  const activate = useCallback(async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(farmBoosterV3Contract, 'activate', [tokenId], { gasLimit: BOOSTED_FARM_V3_GAS_LIMIT })
    })
  ...
  const deactivate = useCallback(async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(farmBoosterV3Contract, 'deactive', [tokenId], { gasLimit: BOOSTED_FARM_V3_GAS_LIMIT })
    })
