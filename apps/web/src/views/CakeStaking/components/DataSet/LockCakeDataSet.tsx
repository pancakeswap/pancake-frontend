...
export const LockCakeDataSet = () => {
  ...
  const veCakeAmount = useMemo(() => {
    const remainLockWeeks = Math.floor((cakeUnlockTime - currentTimestamp) / WEEK)
    return getBalanceAmount(veCakeBalance).plus(getVeCakeAmount(cakeLockAmount, remainLockWeeks))
