...
const AllHistoryCard = () => {
  ...
  const [latestRoundId, setLatestRoundId] = useState<number | undefined>()
  ...
  useEffect(() => {
    if (currentPotteryId) {
      ...
      const mostRecentFinishedRoundId =
        currentPotteryIdAsInt && currentPotteryIdAsInt >= 0 ? currentPotteryIdAsInt + 1 : undefined
      setLatestRoundId(mostRecentFinishedRoundId)
      setSelectedRoundId(mostRecentFinishedRoundId !== undefined ? mostRecentFinishedRoundId.toString() : '')
    }
  }, [currentPotteryId])
  ...
  const handleInputChange = (event) => {
    ...
    if (value) {
      ...
      if (latestRoundId !== undefined && parseInt(value, 10) >= latestRoundId) {
