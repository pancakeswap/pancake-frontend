...
import { Flex, Link, Svg, SvgProps, Text, useMatchBreakpoints } from '@pancakeswap/uikit'
import StyledButton from '@pancakeswap/uikit/components/Button/StyledButton'
...
import { FourMemeBg, MbolieFourMemeBg } from './images'
...
`
const BgWrapper2 = styled.div<{ right: number; top: number }>`
  position: absolute;
  //   right: -50px;
  top: -28px;
  overflow: hidden;
...
const FourMemeBanner = () => {
  ...
  return (
    <S.Wrapper
      style={{
        background: ` linear-gradient(0deg, rgba(80, 232, 146, 1) -4.95%, #020D18 6.81%, #020D18 73.44%), linear-gradient(90deg, #6EF75A 0%, #28D9CF 100%), linear-gradient(0deg, #020D18 0%, #020D18 100%), radial-gradient(113.12% 140.14% at 26.47% 0%, #94F272 0%, #77E0A7 33.21%, #65D2D9 79.02%)`,
    ...
    >
      <S.Inner>
        <S.LeftWrapper position="relative" style={{ zIndex: 3, justifyContent: isMobile ? 'flex-start' : 'center' }}>
          ...
          <Header>{t('Rocker launching your meme career')}</Header>
          {/* {!isMobile && <SubTitle>{t('Rocket Launching Your meme career')}</SubTitle>} */}
          <Flex marginTop="30px">
            <Link
            ...
            >
              <StyledButton
                variant="tertiary"
                style={{ background: 'rgba(80, 232, 146, 1)' }}
        ...
        {width > 870 && (
          <BgWrapper>
            <Image src={FourMemeBg} alt="test-img" width={564} height={224} placeholder="blur" />
          </BgWrapper>
        )}
        {width < 870 && (
          <BgWrapper2 right={width > 630 ? 50 : -20} top={width > 630 ? 1 : -10}>
            <Image src={MbolieFourMemeBg} alt="test-img" width={272} height={224} placeholder="blur" />
          </BgWrapper2>
        )}
...
export default memo(FourMemeBanner)
