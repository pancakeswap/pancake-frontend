...
import { TEN, ZERO } from './bigNumber'
...
export const sousStake = async (sousChefContract, amount, decimals = 18, account) => {
  return sousChefContract.methods
    .deposit(new BigNumber(amount).times(TEN.pow(decimals)).toString())
    .send({ from: account, gas: 200000 })
    .on('transactionHash', (tx) => {
      return tx.transactionHash
    })
...
export const sousUnstake = async (sousChefContract, amount, decimals = 18, account) => {
  ...
  return sousChefContract.methods
    .withdraw(new BigNumber(amount).times(TEN.pow(decimals)).toString())
    .send({ from: account, gas: 200000 })
    .on('transactionHash', (tx) => {
      return tx.transactionHash
    })
...
export const soushHarvestBnb = async (sousChefContract, account) => {
  return sousChefContract.methods
    .deposit()
    .send({ from: account, gas: 200000, value: ZERO })
    .on('transactionHash', (tx) => {
      return tx.transactionHash
    })
}
