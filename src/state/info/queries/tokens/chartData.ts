...
const getTokenChartDataETH = async (
  skip: number,
  address: string,
  ...
  try {
    ...
    `
    const { tokenDayDatas } = await infoClientETH.request<TokenDayDatasResponse>(query, {
      startTime: PCS_V2_START,
      skip,
      address,
    })
...
export const fetchTokenChartDataETH = async (address: string): Promise<{ data?: ChartEntry[]; error: boolean }> => {
  return fetchChartData(getTokenChartDataETH, address)
}
