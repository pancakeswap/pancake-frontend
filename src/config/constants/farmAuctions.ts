...
import { WBNB, BUSD } from './tokens'
import { FarmAuctionBidderConfig } from './types'

const getLpAddress = (token0: string, token1: Token) => {
  const token0asToken = new Token(ChainId.MAINNET, token0, 18)
  return Pair.getAddress(token0asToken, token1)
}
...
export const whitelistedBidders: FarmAuctionBidderConfig[] = [
  ...
  },
  {
    ...
    quoteToken: BUSD[ChainId.MAINNET],
    tokenName: 'RAMP DEFI',
