...
const NftProfilePage = () => {
  ...
  const { nfts, isLoading: isNftLoading } = useNftsForAddress(accountAddress, profile, isProfileFetching, lastUpdated)

  return (
    <>
      <SubMenu />
      {isConnectedProfile ? (
        <UserNfts nfts={nfts} isLoading={isNftLoading} onSuccess={setLastUpdated} />
      ) : (
