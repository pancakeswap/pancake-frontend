import { useEffect, useReducer } from 'react'

export interface Ticket {
  id: number
  numbers: string[]
  duplicateWith: number[]
  isComplete: boolean
}

export interface TicketsState {
  tickets: Ticket[]
  allComplete: boolean
  containsDuplicates: boolean
}

const getInitialState = ({
  amount,
  preloadState,
}: {
  amount: number
  preloadState: TicketsState | null
}): TicketsState => {
  if (!preloadState) {
    const tickets = Array.from({ length: amount }, (_, i) => i + 1).map((index) => ({
      id: index,
      numbers: ['', '', '', '', '', ''],
      duplicateWith: [],
      isComplete: false,
    }))
    return {
      tickets,
      allComplete: false,
      containsDuplicates: false,
    }
  }
  const existingTicketsAmount = preloadState.tickets.length
  if (existingTicketsAmount < amount) {
    const additionalTicketsAmount = amount - existingTicketsAmount
    const additionalTickets = Array.from({ length: additionalTicketsAmount }, (_, i) => i + 1).map((index) => ({
      id: existingTicketsAmount + index,
      numbers: ['', '', '', '', '', ''],
      duplicateWith: [],
      isComplete: false,
    }))
    return {
      tickets: [...preloadState.tickets, ...additionalTickets],
      allComplete: false,
      containsDuplicates: preloadState.containsDuplicates,
    }
  }
  if (existingTicketsAmount > amount) {
    return {
      tickets: preloadState.tickets.slice(0, amount),
      allComplete: false,
      containsDuplicates: preloadState.containsDuplicates,
    }
  }
  return preloadState
}

const reducer = (state: TicketsState, action: any) => {
  switch (action.type) {
    case 'updateTicket': {
      const tickets = [...state.tickets]
      const { ticketId, newNumbers } = action.payload

      const newDuplicates = state.tickets.filter(
        (ticket) => ticket.id !== ticketId && ticket.isComplete && ticket.numbers.join('') === newNumbers.join(''),
      )

      // If ticket was duplicate but not duplicate anymore with this update
      // go through previously considered duplicates and remove id of this ticket
      // from their duplicateWith array
      const prevDuplicates = tickets[ticketId - 1].duplicateWith
      prevDuplicates.forEach((prevTicketId) => {
        if (!newDuplicates.map(({ id }) => id).includes(prevTicketId)) {
          const dupsToUpdate = [...tickets[prevTicketId - 1].duplicateWith]
          const indexToRemvoe = dupsToUpdate.findIndex((id) => id === ticketId)
          dupsToUpdate.splice(indexToRemvoe, 1)
          tickets[prevTicketId - 1] = {
            ...tickets[prevTicketId - 1],
            duplicateWith: dupsToUpdate,
          }
        }
      })

      // If found duplicates - update their duplicateWith array
      if (newDuplicates.length !== 0) {
        newDuplicates.forEach((duplicate) => {
          tickets[duplicate.id - 1] = {
            ...duplicate,
            duplicateWith: [...duplicate.duplicateWith, ticketId],
          }
        })
      }

      const updatedTicket = {
        id: ticketId,
        numbers: newNumbers,
        duplicateWith: newDuplicates.map((ticket) => ticket.id),
        isComplete: newNumbers.join('').length === 6,
      }
      tickets[ticketId - 1] = updatedTicket

      // Check if all tickets are filled
      const allComplete = tickets.every((ticket) => ticket.isComplete)
      const containsDuplicates = tickets.some((ticket) => ticket.duplicateWith.length !== 0)

      return {
        tickets,
        allComplete,
        containsDuplicates,
      }
    }
    case 'reset':
      return getInitialState({ amount: action.payload.amount, preloadState: action.payload.preloadState })
    default:
      throw new Error()
  }
}

type UpdateTicketAction = (ticketId: number, newNumbers: string[]) => void

export const useTicketsReducer = (
  amount: number,
  preloadState: TicketsState | null,
): [UpdateTicketAction, Ticket[], boolean, boolean] => {
  const [state, dispatch] = useReducer(reducer, { amount, preloadState }, getInitialState)

  useEffect(() => {
    dispatch({ type: 'reset', payload: { amount, preloadState } })
  }, [amount, preloadState])

  const updateTicket = (ticketId: number, newNumbers: string[]) => {
    dispatch({ type: 'updateTicket', payload: { ticketId, newNumbers } })
  }

  return [updateTicket, state.tickets, state.allComplete, state.containsDuplicates]
}
