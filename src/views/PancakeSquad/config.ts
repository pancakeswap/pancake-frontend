...
const getEventStepStatus = ({ eventStatus, saleStatus }: getEventStepStatus): EventStatus => {
  ...
  if (eventStatus.some((status) => status === saleStatus)) return 'live'
  return 'upcoming'
}
...
const getEventText = ({ eventStatus, saleStatus, t }: getEventTextType): string => {
  const eventStepStatus = getEventStepStatus({ eventStatus, saleStatus })
...
const getAltText = ({ startTimestamp, eventStatus, saleStatus, t }: getAltTextType): string | undefined => {
  ...
  if (startTimestamp && eventStepStatus === 'upcoming' && saleStatus === SaleStatusEnum.Sale) {
    const timePeriods = getTimePeriods(startTimestamp)
  ...
  saleStatus
    ? [
        ...
        },
        {
          ...
          text: getEventText({ saleStatus, eventStatus: [SaleStatusEnum.Sale, SaleStatusEnum.DrawingRandomness], t }),
          altText: getAltText({
            t,
            saleStatus,
            eventStatus: [SaleStatusEnum.Sale, SaleStatusEnum.DrawingRandomness],
        ...
        },
        {
          ...
          altText: getAltText({ t, saleStatus, eventStatus: [SaleStatusEnum.Claim], startTimestamp }),
          infoText: t(
