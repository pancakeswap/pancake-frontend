...
import { Button, Modal } from 'greenteaswap-ui-kit'
...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  return (
    <Modal title={TranslateString(450, 'Enter amount of tickets to buy')} onDismiss={onDismiss}>
      ...
      />
      <div>
        <Tips>{TranslateString(999, `1 Ticket = ${LOTTERY_TICKET_PRICE} TEA`, { num: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {TranslateString(
            478,
            'Ticket purchases are final. Your TEA cannot be returned to you after buying tickets.',
          )}
        </Announce>
        <Final>{TranslateString(460, `You will spend: ${cakeCosts(val)} TEA`)}</Final>
      </div>
      <ModalActions>
