...
export abstract class SwapRouter {
  ...
  private static encodeMixedRouteSwap(
    ...
    for (const { route, inputAmount, outputAmount } of trade.swaps) {
      ...
      const mixedRouteIsAllV3 = (routeArg: MixedRouteSDK<Currency, Currency>) => {
        return routeArg.pools.every((pool) => pool instanceof Pool)
      }
  ...
  private static encodeSwaps(
    ...
    const numberOfTrades = trades.reduce(
      (nOfTrades, trade) =>
        nOfTrades + (trade instanceof V3Trade || trade instanceof MixedRouteTrade ? trade.swaps.length : 1),
      0
    )
  ...
  public static swapAndAddCallParameters(
    ...
    // if tokens are native they will be converted to WNATIVE
