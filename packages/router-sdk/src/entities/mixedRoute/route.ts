...
import { Currency, Price, Token, Pair } from '@pancakeswap/sdk'
...
export class MixedRouteSDK<TInput extends Currency, TOutput extends Currency> {
  ...
  public get midPrice(): Price<TInput, TOutput> {
    ...
    const { price } = this.pools.slice(1).reduce(
      ({ nextInput, price: poolPrice }, pool) => {
        return nextInput.equals(pool.token0)
          ? {
              nextInput: pool.token1,
              price: poolPrice.multiply(pool.token0Price),
            }
          : {
              nextInput: pool.token0,
              price: poolPrice.multiply(pool.token1Price),
            }
      },
