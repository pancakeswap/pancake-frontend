...
import { BestTradeOptions, Pool } from '@pancakeswap/v3-sdk'
 ...
 * @notice This class is functionally the same as the `Trade` class in the `@pancakeswap/v3-sdk` package, aside from typing and some input validation.
 * @template TInput The input token, either Ether or an ERC-20
 * @template TOutput The output token, either Ether or an ERC-20
 * @template TTradeType The trade type, either exact input or exact output
 */
export class MixedRouteTrade<TInput extends Currency, TOutput extends Currency, TTradeType extends TradeType> {
  ...
  public static async fromRoute<TInput extends Currency, TOutput extends Currency, TTradeType extends TradeType>(
    ...
    for (let i = 0; i < route.path.length - 1; i++) {
      ...
      const [outputA] = await pool.getOutputAmount(amounts[i])
      amounts[i + 1] = outputA
    }
  ...
  public static async bestTradeExactIn<TInput extends Currency, TOutput extends Currency>(
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      try {
        // eslint-disable-next-line no-await-in-loop
        const [aOut] = await pool.getOutputAmount(amountIn)
      ...
      if (amountOut.currency.isToken && amountOut.currency.equals(tokenOut)) {
        // eslint-disable-next-line no-await-in-loop
        const result = await MixedRouteTrade.fromRoute(
          new MixedRouteSDK([...currentPools, pool], currencyAmountIn.currency, currencyOut),
          currencyAmountIn,
          TradeType.EXACT_INPUT
        )

        sortedInsert(bestTrades, result, maxNumResults, tradeComparator)
