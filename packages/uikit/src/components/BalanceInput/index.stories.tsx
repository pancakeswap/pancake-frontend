...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"WANO" | "USD">("WANO");
  const conversionUnit = editingUnit === "WANO" ? "USD" : "WANO";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "WANO" ? "USD" : "WANO";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "WANO") {
      ...
      setValues({
        WANO: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
