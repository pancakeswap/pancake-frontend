...
describe('Coin', () => {
  ...
  describe('#equals', () => {
    ...
    it('sortBefore', () => {
      ...
      const usdc = new Coin(ChainId.DEVNET, ADDRESS_USDC, 8, 'USDC')
      const busd = new Coin(ChainId.DEVNET, ADDRESS_BUSD, 8, 'BUSD')
      expect(usdc.sortsBefore(busd)).toBe(false)

      const bnb = new Coin(ChainId.DEVNET, ADDRESS_BNB, 8, 'BNB')
