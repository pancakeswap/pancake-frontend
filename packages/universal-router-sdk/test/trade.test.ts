...
describe('PancakeSwap Universal Router Trade', () => {
  ...
  describe('multi-route', () => {
    ...
    it('should encode a split exactOutput with 2 routes: v2 ETH-USDC & v3 ETH-USDC', async () => {
      const amountOut = parseEther('1')
      const v2TradeRoute = new V2Trade(
        new V2Route([WETH_USDC_V2], ETHER, USDC),
        CurrencyAmount.fromRawAmount(USDC, amountOut),
      ...
      const options = swapOptions({})
      const routes = [...v2Trade.routes, ...v3Trade.routes]
      const trade: SmartRouterTrade<TradeType.EXACT_OUTPUT> = {
        tradeType: TradeType.EXACT_OUTPUT,
        inputAmount: CurrencyAmount.fromRawAmount(
          ETHER,
          routes.reduce((acc, r) => acc + r.inputAmount.quotient, 0n),
        ),
        outputAmount: CurrencyAmount.fromRawAmount(USDC, amountOut * 2n),
        routes,
        gasEstimate: 0n,
        gasEstimateInUSD: CurrencyAmount.fromRawAmount(ETHER, 0),
      }

      const { calldata, value } = PancakeSwapUniversalRouter.swapERC20CallParameters(trade, options)
      expect(BigInt(value)).toEqual(SmartRouter.maximumAmountIn(trade, options.slippageTolerance).quotient)
      expect(calldata).toMatchSnapshot()
      ...
      expect(decodedCommands[1].args[0].value).toEqual(SENDER_AS_RECIPIENT)
      expect(decodedCommands[1].args[1].name).toEqual('amountOut')
      expect(decodedCommands[1].args[1].value).toEqual(v2TradeRoute.outputAmount.quotient)
      expect(decodedCommands[1].args[2].name).toEqual('amountInMax')
      expect(decodedCommands[1].args[2].value).toEqual(v2TradeRoute.maximumAmountIn(options.slippageTolerance).quotient)
      expect(decodedCommands[1].args[3].name).toEqual('path')
      expect((decodedCommands[1].args[3].value as string[]).length).toEqual(2)
      expect(decodedCommands[1].args[4].value).toEqual(false)

      expect(decodedCommands[2].command).toEqual(CommandType[CommandType.V3_SWAP_EXACT_OUT])
      expect(decodedCommands[2].args[0].name).toEqual('recipient')
      expect(decodedCommands[2].args[0].value).toEqual(SENDER_AS_RECIPIENT)
      expect(decodedCommands[2].args[1].name).toEqual('amountOut')
      expect(decodedCommands[2].args[1].value).toEqual(v3TradeRoute.outputAmount.quotient)
      expect(decodedCommands[2].args[2].name).toEqual('amountInMax')
      expect(decodedCommands[2].args[2].value).toEqual(v3TradeRoute.maximumAmountIn(options.slippageTolerance).quotient)
      expect(decodedCommands[2].args[4].name).toEqual('payerIsUser')
      expect(decodedCommands[2].args[4].value).toEqual(false)

      expect(decodedCommands[3].command).toEqual(CommandType[CommandType.UNWRAP_WETH])
      expect(decodedCommands[3].args[0].name).toEqual('recipient')
      expect(decodedCommands[3].args[0].value).toEqual(SENDER_AS_RECIPIENT)
    })
    ...
    it('should encode a split exactOutput with 3 routes: v2 ETH-USDC & v3 ETH-USDC & v3 ETH-USDC', async () => {
      const amountOut = parseEther('2')
      const v2TradeRoute = new V2Trade(
        new V2Route([WETH_USDC_V2], ETHER, USDC),
        CurrencyAmount.fromRawAmount(USDC, (amountOut * 5000n) / 10000n),
        TradeType.EXACT_OUTPUT,
      )
      const v2Trade = buildV2Trade(v2TradeRoute, [
        {
      ...
      const v3TradeRoute = await V3Trade.fromRoute(
        new V3Route([WETH_USDC_V3_MEDIUM], ETHER, USDC),
        CurrencyAmount.fromRawAmount(USDC, (amountOut * 2500n) / 10000n),
        TradeType.EXACT_OUTPUT,
      )
      const v3Trade = buildV3Trade(v3TradeRoute, [convertPoolToV3Pool(WETH_USDC_V3_MEDIUM)])
      ...
      expect(decodedCommands[3].command).toEqual(CommandType[CommandType.V3_SWAP_EXACT_OUT])
      expect(decodedCommands[3].args[0].name).toEqual('recipient')
      expect(decodedCommands[3].args[0].value).toEqual(SENDER_AS_RECIPIENT)
      expect(decodedCommands[3].args[1].name).toEqual('amountOut')
      expect(decodedCommands[3].args[1].value).toEqual(v3TradeRoute2.outputAmount.quotient)
      expect(decodedCommands[3].args[2].name).toEqual('amountInMax')
      expect(decodedCommands[3].args[2].value).toEqual(
        v3TradeRoute2.maximumAmountIn(options.slippageTolerance).quotient,
      )
      expect(decodedCommands[3].args[4].name).toEqual('payerIsUser')
      expect(decodedCommands[3].args[4].value).toEqual(false)
...
describe('PancakeSwap StableSwap Through Universal Router, BSC Network Only', () => {
  ...
  describe('multi-route', () => {
    ...
    // TODO: check if stable swap support exact output
    // it.only('should encode a multi-route exactOutput v2 USDT-USDC & v3 USDT-USDC & stable USDT-USDC', async () => {
    //   const amountOut = parseUnits('1000', 6)
    //   const v2TradeRoute = new V2Trade(
    //     new V2Route([USDC_USDT_V2], USDT, USDC),
    //     CurrencyAmount.fromRawAmount(USDC, amountOut),
    ...
    //   const stableTrade = buildStableTrade(USDT, USDC, CurrencyAmount.fromRawAmount(USDC, amountOut), [
    //     await getStablePool(USDT, USDC, getPublicClient, liquidity),
    //   ])
    ...
    //       routes.reduce((acc, r) => acc + r.inputAmount.quotient, 0n),
    //     ),
    //     outputAmount: CurrencyAmount.fromRawAmount(USDC, amountOut * 3n),
    //     routes,
    //     gasEstimate: 0n,
    //     gasEstimateInUSD: CurrencyAmount.fromRawAmount(USDT, 0),
    //   }
    ...
    //   expect(decodedCommands[0].args[1].value).toEqual(amountOut)
    //   expect(decodedCommands[0].args[2].name).toEqual('amountInMax')
    //   expect(decodedCommands[0].args[2].value).toEqual(v2TradeRoute.maximumAmountIn(options.slippageTolerance).quotient)
    //   expect(decodedCommands[0].args[3].name).toEqual('path')
    //   expect((decodedCommands[0].args[3].value as string[]).length).toEqual(2)
    //   expect(decodedCommands[0].args[4].value).toEqual(true)
    ...
    //   expect(decodedCommands[1].args[1].value).toEqual(amountOut)
    //   expect(decodedCommands[1].args[2].name).toEqual('amountInMax')
    //   expect(decodedCommands[1].args[2].value).toEqual(v3TradeRoute.maximumAmountIn(options.slippageTolerance).quotient)
    //   expect(decodedCommands[1].args[4].name).toEqual('payerIsUser')
    //   expect(decodedCommands[1].args[4].value).toEqual(true)

    //   expect(decodedCommands[2].command).toEqual(CommandType[CommandType.STABLE_SWAP_EXACT_OUT])
    //   expect(decodedCommands[2].args[0].name).toEqual('recipient')
    //   expect(decodedCommands[2].args[0].value).toEqual(SENDER_AS_RECIPIENT)
    //   expect(decodedCommands[2].args[1].name).toEqual('amountOut')
    //   expect(decodedCommands[2].args[1].value).toEqual(amountOut)
    //   expect(decodedCommands[2].args[2].name).toEqual('amountInMax')
    //   expect(decodedCommands[2].args[2].value).toEqual(
    //     SmartRouter.maximumAmountIn(stableTrade, options.slippageTolerance).quotient,
    //   )
    //   expect(decodedCommands[2].args[3].name).toEqual('path')
    //   expect((decodedCommands[2].args[3].value as string[]).length).toEqual(2)
    //   expect(decodedCommands[2].args[5].name).toEqual('payerIsUser')
