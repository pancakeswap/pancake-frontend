...
export const Details = memo(function Details({
  ...
  const details = isExpanded ? (
    <Box px="8px">
      <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows="repeat(2, auto)" mb="8px">
        ...
        <Text small bold textAlign="right" color="dark">
          ${formatAmount(+totalYield)}
        </Text>
        <Text color="dark" small style={{ textIndent: "1em" }}>
          {t("LP Fee Yield")}
        </Text>
        <Text small color="dark" textAlign="right">
      ...
      <BulletList>
        <li>
          <Text fontSize="12px" textAlign="center" color="dark" display="inline" lineHeight={1.1}>
            {t(
  ...
  return (
    <StyledFooter p="16px" flexDirection="column">
      <ExpandableLabel expanded={isExpanded} variant="tertiary" onClick={() => setIsExpanded((prev) => !prev)}>
