import { ChainId } from '@pancakeswap/chains'
import {
  arbitrumTokens,
  baseTokens,
  bscTokens,
  ethereumTokens,
  lineaTokens,
  opBnbTokens,
  polygonZkEvmTokens,
  zksyncTokens,
} from '@pancakeswap/tokens'
import { FeeAmount } from '@pancakeswap/v3-sdk'
import { GaugeConfig, GaugeType } from '../../types'

export const CONFIG_PROD: GaugeConfig[] = [
  {
    gid: 0,
    address: '0x133B3D95bAD5405d14d53473671200e9342896BF',
    pairName: 'CAKE-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 1,
    address: '0x7f51c8AaA6B0599aBd16674e2b17FEc7a9f674A1',
    pairName: 'CAKE-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 2,
    address: '0x85FAac652b707FDf6907EF726751087F9E0b6687',
    pairName: 'BUSD-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.busd.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 3,
    address: '0x36696169C63e42cd08ce11f5deeBbCeBae652050',
    pairName: 'USDT-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 4,
    address: '0x369482C78baD380a036cAB827fE677C1903d1523',
    pairName: 'BTCB-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 5,
    address: '0x46Cf1cF8c69595804ba91dFdd8d6b960c9B0a7C4',
    pairName: 'BTCB-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 6,
    address: '0xD4dCA84E1808da3354924cD243c66828cf775470',
    pairName: 'BTCB-ETH',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 7,
    address: '0xFC75f4E78bf71eD5066dB9ca771D4CcB7C1264E0',
    pairName: 'BTCB-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 8,
    address: '0x7d05c84581f0C41AD80ddf677A510360bae09a5A',
    pairName: 'ETH-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 9,
    address: '0x92b7807bF19b7DDdf89b706143896d05228f3121',
    pairName: 'USDC-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdc.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 10,
    address: '0x22536030B9cE783B6Ddfb9a39ac7F439f568E5e6',
    pairName: 'USDC-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdc.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 11,
    address: '0x4f3126d5DE26413AbDCF6948943FB9D0847d9818',
    pairName: 'BUSD-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.busd.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 12,
    address: '0x66E9AcBA7C3a82C8EE02fAF3E325Dd55D6581a8c',
    pairName: 'ETH-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 13,
    address: '0x9f6EB6903C1277c8f02d71F8814dc9998199af1D',
    pairName: 'CAKE-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.MEDIUM,
  },

  {
    gid: 14,
    address: '0x539e0EBfffd39e54A0f7E5F8FEc40ade7933A664',
    pairName: 'USDC-ETH',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdc.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 15,
    address: '0x0E1893BEEb4d0913d26B9614B18Aea29c56d94b9',
    pairName: 'LINK-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.link.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 16,
    address: '0xd15B00E81F98A7DB25f1dC1BA6E983a4316c4CaC',
    pairName: 'XRP-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.xrp.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 17,
    address: '0x673516E510d702Ab5F2bBf0c6B545111a85f7ea7',
    pairName: 'ADA-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ada.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 18,
    address: '0x62F0546cBcd684F7C394D8549119e072527C41Bc',
    pairName: 'DOT-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.dot.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 19,
    address: '0x61837a8a78F42dC6cfEd457c4eC1114F5e2d90f4',
    pairName: 'ankrETH-ETH',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ankrETH.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 20,
    address: '0xCf57DaADfBE05A04440C502967cE5209F64747eB',
    pairName: 'ankrBNB-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ankrbnb.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 21,
    address: '0x07003daEbc432ecec26309cCd1391BBBF06cC890',
    pairName: 'GQ-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.gq.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 22,
    address: '0xD10612A288Bd5024Db6a47663750996d176130Fe',
    pairName: 'AXL-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.axl.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 23,
    address: '0x088464e4E8CC54BF91180cBb8c61C68AeCC74166',
    pairName: 'MGP-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.mgp.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 24,
    address: '0xb4E9DeA6105089f15685508B8EF2e7f7F5A1B16D',
    pairName: 'UNW-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.unw.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 25,
    address: '0xAE7a3D9bed2ba4ef9c134FF4BAeE33655AE5DE6f',
    pairName: 'stkBNB-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.stkbnb.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 26,
    address: '0x2dA32920A775CF121004551AbC92F385B3C0Dab9',
    pairName: 'CHAMP-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.champ.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 27,
    address: '0x63ca58e7c6bF06B06cBbEc2a83bf6AA8f8f9f77B',
    pairName: 'ZBC-CAKE',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.zbc.address,
    token1Address: bscTokens.cake.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 28,
    address: '0x379044E32f5A162233E82de19DA852255d0951b8',
    pairName: 'WBETH-ETH',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.wbeth.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 29,
    address: '0x6425bC30D0751aF5181fC74a50e760b0e4a19811',
    pairName: 'EDU-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.edu.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 30,
    address: '0xFB5C2D2f2cF7741ba1A7Be2FfAbED248BD9b888e',
    pairName: 'EDU-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.edu.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 31,
    address: '0x08eAbc3d13Fb4bdFFD1F42a5644C1c826aCF62c0',
    pairName: 'PEEL-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.peel.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 32,
    address: '0x77B27c351B13Dc6a8A16Cc1d2E9D5e7F9873702E',
    pairName: 'BNBX-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.bnbx.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 33,
    address: '0x729c9a7E1f642Eab4019dfb0d467f4a9838E7cbD',
    pairName: 'GAL-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.gal.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 34,
    address: '0x4e1f9aDf96dBA6Dc09c973228c286568F1315ea8',
    pairName: 'ID-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.id.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 35,
    address: '0x3Cb75d72401d2DBA4349C696824F1397277d6a11',
    pairName: 'USH-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ush.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 36,
    address: '0xC2B3dbBF26d43617036b0ebA53aD2DBD945adebf',
    pairName: 'C98-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.c98.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 37,
    address: '0xbba8f85c3cEDdF73dB4de17D31608d640Eaea416',
    pairName: 'PEPE-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.pepe.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 38,
    address: '0x466e7d53e23620a24db23E2B1f8BD10Ff52116Cd',
    pairName: 'CSIX-CAKE',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.csix.address,
    token1Address: bscTokens.cake.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 39,
    address: '0x7B1db35fbd95548777B9079527e8fa2a70fb2CE0',
    pairName: 'agEUR-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eura.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 40,
    address: '0x73D69D55893d6c97DCA44AF2Aa85B688C0242d7f',
    pairName: 'PLAY-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.play.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 41,
    address: '0xd881D9d0e0767719701305c614903F555d589586',
    pairName: 'TUSD-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.tusd.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 42,
    address: '0x436380Bd2404aBA3f20c0ea68F343f5a174532a7',
    pairName: 'xALGO-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.xalgo.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 43,
    address: '0x85BeA4fBC57fA22b41Fb1632f0D9a6A99390fd0a',
    pairName: 'PENDLE-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.pendle.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 44,
    address: '0x803036AC78752EF599EC75c500ac8B0Ac0bE67dF',
    pairName: 'axlUSDC-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.axlusdc.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 45,
    address: '0xD0e226f674bBf064f54aB47F42473fF80DB98CBA',
    pairName: 'ETH-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 46,
    address: '0x6bbc40579ad1BBD243895cA0ACB086BB6300d636',
    pairName: 'BTCB-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 47,
    address: '0x6ee3eE9C3395BbD136B6076A70Cb6cFF241c0E24',
    pairName: 'BTCB-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 48,
    address: '0x647D99772863e09f47435782cbb6C96eC4A75f12',
    pairName: 'UNI-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.uni.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 49,
    address: '0xdf0c1C30e8C1aE3f189f6E6ef248d71977F7BE29',
    pairName: 'TUSD-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.tusd.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 50,
    address: '0xBe43E64Cd61E0b9207A6beE93e2149317A604326',
    pairName: 'BNB-lisUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.bnb.address,
    token1Address: bscTokens.hay.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 51,
    address: '0x06E2D4002a693812A6348c91A2bEccf4E926FF2f',
    pairName: 'BTCB-lisUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.hay.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 52,
    address: '0xfdFCdE34d2038EBeDe62e95C65b1492C28c990C9',
    pairName: 'ETH-lisUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.hay.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 53,
    address: '0x9474e972F49605315763c296B122CBB998b615Cf',
    pairName: 'slisBNB-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.snbnb.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 54,
    address: '0xD465D9C13C43003f5B874e0D96A6030336Ed50eB',
    pairName: 'DCK-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.dck.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 55,
    address: '0xce6160bB594fC055c943F59De92ceE30b8c6B32c',
    pairName: 'DOGE-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.doge.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 56,
    address: '0xE6be850a43ae64c68754845ea444DE7d3fe761AB',
    pairName: 'OLE-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ole.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 57,
    address: '0x13f0D0dF1f347E7246Ba16866d2562Bb2B337d0c',
    pairName: 'WMX-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.wmx.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 58,
    address: '0xd9d0aeD9822e3D7C67b9c13A18dE070f86cdF2e5',
    pairName: 'DAR-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.dar.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 59,
    address: '0x3ccCef8d9D515eC7F59eb69aD06C22265cC95ea9',
    pairName: 'unshETH-ETH',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.unshETH.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 60,
    address: '0x846BD025527c8427809E11D0B0a9cE50F149D5d5',
    pairName: 'CYBER-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.cyber.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 61,
    address: '0x3B7761632240c4BBec6deE27E10d491De9AA669B',
    pairName: 'GMT-USDC',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.gmt.address,
    token1Address: bscTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 62,
    address: '0x77E4ba48091f23a8a54b3e2c72F17be58cC2D137',
    pairName: 'LVL-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.lvl.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 63,
    address: '0x29e13e65E8e2160dBd391bA3cee55E7dDe2A386c',
    pairName: 'SFUND-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.sfund.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 64,
    address: '0x4f55423de1049d3CBfDC72f8A40f8A6f554f92aa',
    pairName: 'RACA-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.raca.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 65,
    address: '0x293c665b9b98Cd51D6454C72529Fe17A1Cf1f504',
    pairName: 'HIGH-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.high.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 66,
    address: '0xbB87F33d9f43Ec3fe75502958408262e7043B2E8',
    pairName: 'ARV-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.arv.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 67,
    address: '0x6dba30870BC46BD6d2289E1827Cc415F6Fa23E1a',
    pairName: 'HFT-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.hft.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 68,
    address: '0x635c26c473BB0686D403247477CC648a7C6edc2e',
    pairName: 'CHR-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.chr.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 69,
    address: '0x12E79eb21dCc5852F9c6AC1736d977312925da33',
    pairName: 'lisUSD-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.hay.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 70,
    address: '0xd8CF0de0387A9c16acF53384c89632678D77F311',
    pairName: 'WNCG-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.wncg.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 71,
    address: '0x5a1Fe6D5026CC3736Cfb7316dbCFbf63D82cC789',
    pairName: 'MBX-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.mbx.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 72,
    address: '0x66bAA9E43e64c8A85bC3c04dEBf9E7686BE5b09C',
    pairName: 'MBX-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.mbx.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 73,
    address: '0x302e26e9bda986709B5F504D3426c2310e6383c6',
    pairName: 'XCAD-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.xcad.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 74,
    address: '0x64ebB904e169cB94e9788FcB68283B4C894ED881',
    pairName: 'SFP-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.sfp.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 75,
    address: '0xE3cBe4Dd1BD2F7101f17D586F44bAb944091D383',
    pairName: 'LTC-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ltc.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 76,
    address: '0xB55A0B97B7D9Ebe4208b08AB00feC0C419cc32A3',
    pairName: 'RDNT-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.rdnt.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 77,
    address: '0x0004222c2075E9A1291E41f1cA4C8d32141dB501',
    pairName: 'MBOX-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.mbox.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 78,
    address: '0xDcccC7d0B02C837d1B8D8a8D5E2683387bc2b910',
    pairName: 'WOM-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.wom.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 79,
    address: '0x81Bef404f5C93d99ed04Ed55488c99722CDd7A50',
    pairName: 'AXS-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.axs.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 80,
    address: '0xa98D8a5867D664B7A758652146fd93a7dE40eE82',
    pairName: 'TRX-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.trxv2.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 81,
    address: '0x77d5b2560e4B84b3fC58875Cb0133F39560e8AE3',
    pairName: 'XVS-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.xvs.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 82,
    address: '0xE4e695FA53598dA586F798A9844A3b03d86f421e',
    pairName: 'BTT-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btt.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 83,
    address: '0xcfe783e16c9a8C74F2be9BCEb2339769439061Bf',
    pairName: 'ALPACA-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.alpaca.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 84,
    address: '0x832EeBF89F99aACcf6640fe6b5E838066c630Fbe',
    pairName: 'CHESS-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.chess.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 85,
    address: '0x8cCB4544b3030dACF3d4D71C658f04e8688e25b1',
    pairName: 'TWT-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.twt.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 86,
    address: '0xBe141893E4c6AD9272e8C04BAB7E6a10604501a5',
    pairName: 'ETH-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 87,
    address: '0xbF72B6485E4b31601aFe7B0a1210Be2004D2B1d6',
    pairName: 'FDUSD-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.fdusd.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 88,
    address: '0x4BBA1018b967e59220b22Ca03f68821A3276c9a6',
    pairName: 'BTCB-ETH',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 89,
    address: '0x5F16320FA36559ca7bc785834Ba77105154DC40b',
    pairName: 'STG-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.stg.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 90,
    address: '0x89A6be1ec107C911C3F2A1112f049F876Ce033c9',
    pairName: 'STG-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.stg.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },

  {
    gid: 91,
    address: '0x9cac9745731d1Cf2B483f257745A512f0938DD01',
    pairName: 'CAKE Pool',
    chainId: ChainId.BSC,
    type: GaugeType.VeCakePool,
  },

  {
    gid: 92,
    address: '0x0eD7e52944161450477ee417DE9Cd3a859b14fD0',
    pairName: 'CAKE-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 93,
    address: '0xA39Af17CE4a8eb807E076805Da1e2B8EA7D0755b',
    pairName: 'CAKE-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 94,
    address: '0x7EB5D86FD78f3852a3e0e064f2842d45a3dB6EA2',
    pairName: 'XVS-WBNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.xvs.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 95,
    address: '0x346575fC7f07E6994D76199E41D13dC1575322E1',
    pairName: 'RDNT-WBNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.rdnt.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 96,
    address: '0x8FA59693458289914dB0097F5F366d771B7a7C3F',
    pairName: 'MBOX-WBNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.mbox.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 97,
    address: '0x942b294e59a8c47a0F7F20DF105B082710F7C305',
    pairName: 'SFP-WBNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.sfp.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 98,
    address: '0xa0D4e270D9EB4E41f7aB02337c21692D7eECCCB0',
    pairName: 'SABLE-WBNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.sable.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 99,
    address: '0x74fA517715C4ec65EF01d55ad5335f90dce7CC87',
    pairName: 'SFUND-WBNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.sfund.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 100,
    address: '0x3DcB1787a95D2ea0Eb7d00887704EeBF0D79bb13',
    pairName: 'TWT-WBNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.twt.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 101,
    address: '0xBCEA09e9e882eC2Bb6dCE07c4e6669968846CaBD',
    pairName: 'STG-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.stg.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 102,
    address: '0xAf839f4D3620a1EED00cCc21dDC01119C26a75E1',
    pairName: 'APX-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.apx.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 103,
    address: '0xa0Ee789a8F581CB92dD9742ed0B5d54a0916976C',
    pairName: 'APX-BUSD',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.apx.address,
    token1Address: bscTokens.busd.address,
    feeTier: FeeAmount.MEDIUM,
  },

  {
    gid: 104,
    address: '0xb1Da7D2C257c5700612BdE35C8d7187dc80d79f1',
    pairName: 'lisUSD-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.hay.address, bscTokens.usdt.address],
    killed: true,
  },
  {
    gid: 105,
    address: '0x1ac1A8FEaAEa1900C4166dEeed0C11cC10669D36',
    pairName: 'USDC-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.usdc.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 106,
    address: '0x6CA298D2983aB03Aa1dA7679389D955A4eFEE15C',
    pairName: 'USDT-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.usdt.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 107,
    address: '0x04c8577958CcC170EB3d2CCa76F9d51bc6E42D8f',
    pairName: 'USDT-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.usdt.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 108,
    address: '0x9b5699D18DFF51fc65fB8ad6F70d93287C36349f',
    pairName: 'WBTC-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wbtc.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 109,
    address: '0x517F451b0A9E1b87Dc0Ae98A05Ee033C3310F046',
    pairName: 'CAKE-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.cake.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 110,
    address: '0x11A6713B702817DB0Aa0964D1AfEe4E641319732',
    pairName: 'CAKE-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.cake.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 111,
    address: '0xD9e497BD8f491fE163b42A62c296FB54CaEA74B7',
    pairName: 'DAI-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.dai.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 112,
    address: '0x34b8AB3a392d54D839dcDBd5Cd1330aBB24bE167',
    pairName: 'LDO-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.ldo.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 113,
    address: '0x7ca3EdB2c8fb3e657E282e67F4008d658aA161D2',
    pairName: 'LINK-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.link.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 114,
    address: '0x8579630AC9c53CFEb5167f90Af90d2c0d52ED09c',
    pairName: 'MATIC-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.matic.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 115,
    address: '0x4F64951A6583D56004fF6310834C70d182142A07',
    pairName: 'wstETH-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wstETH.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 116,
    address: '0x7524Fe020EDcD072EE98126b49Fa65Eb85F8C44C',
    pairName: 'STG-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.stg.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 117,
    address: '0x372dACe050C15879F01966f0b7efb667dD3151Ad',
    pairName: 'FXS-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.fxs.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 118,
    address: '0x5C9c6F39Ce25cc6d0F39410F890933A1476FB1b0',
    pairName: 'frxETH-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.frxETH.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 119,
    address: '0xCc76f26309E5cb9D18e50DD809074Bf69C341a41',
    pairName: 'RPL-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.rpl.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 120,
    address: '0x2201d2400d30BFD8172104B4ad046d019CA4E7bd',
    pairName: 'rETH-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.rETH.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 121,
    address: '0x66356491821A83431BE84F62Eb9fb5Ad67015274',
    pairName: 'ankrETH-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.ankrETH.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 122,
    address: '0xCCD1d21fcE02959F4F51DDc4505eA154aEBE7F1b',
    pairName: 'cbETH-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.cbEth.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 123,
    address: '0x3364f7925B3E499ec45F6a0C6F744912Fb7394Cf',
    pairName: 'AXL-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.axl.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 124,
    address: '0x6E229C972d9F69c15Bdc7B07f385D2025225E72b',
    pairName: 'MASK-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.mask.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 125,
    address: '0x5145755c0535198eec1642DC0cc96225fb28263D',
    pairName: 'WETH-WNCG',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.wncg.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 126,
    address: '0x402B2BCeb1415F48B413752cC0E27D76ff34ddEb',
    pairName: 'WBETH-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wbeth.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 127,
    address: '0x3202AcfD55232f3706aa81a4F18A98686B5e1d1B',
    pairName: 'PEPE-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.pepe.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 128,
    address: '0xC7F25e2FcC474816efFd9be316F2E51cCef90Ceb',
    pairName: 'BLUR-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.blur.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 129,
    address: '0x392d0F0B7Fe5161Db89f2DB87d33a20682C12A2B',
    pairName: 'ENS-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.ens.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 130,
    address: '0x32600e01DfaeBad27Dac6B68902Abc082219b526',
    pairName: 'FUSE-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.fuse.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 131,
    address: '0x0d917a3a01389689a14C65dc1E990d68C437358A',
    pairName: 'rETH-WBTC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.rETH.address,
    token1Address: ethereumTokens.wbtc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 132,
    address: '0x6C2348DEeC1508724C0207a84d21cC5EE88231C8',
    pairName: 'TUSD-USDT',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.tusd.address,
    token1Address: ethereumTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 133,
    address: '0x9d6371979d2368dbE9480F4822Ed105f03898765',
    pairName: 'CANTO-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.canto.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 134,
    address: '0xCAD4b51069a150a77D3a1d381d2D768769F7D195',
    pairName: 'PENDLE-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.pendle.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 135,
    address: '0x46027b00196275Fb8215a622A44d1269b81cE71a',
    pairName: 'WLD-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wld.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 136,
    address: '0x5e3FE73361e9Ef3706dD4cbDFc6067278C22C769',
    pairName: 'WOM-USDT',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wom.address,
    token1Address: ethereumTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 137,
    address: '0x3fc47BE8264E473dd2B3e80d144F9EfFfc18F438',
    pairName: 'CYBER-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.cyber.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 138,
    address: '0xEa9b2D7ff9aE446ec067e50DF7C09f1Dd055bB71',
    pairName: 'WOO-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.woo.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 139,
    address: '0x4D4c8F2f30e0224889ab578283A844e10B57e0F8',
    pairName: 'ETHx-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.ethx.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 140,
    address: '0x73b9aDC00794260616C51C41997cE0245b3FA012',
    pairName: 'MEME-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.meme.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 141,
    address: '0x3a1b97Fc25fA45832F588ED3bFb2A0f74ddBD4F8',
    pairName: 'wstETH-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wstETH.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 142,
    address: '0x3A2195f4760e89e5B753fd3521a236b6a9f72EBB',
    pairName: 'BTRFLY-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.btrfly.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 143,
    address: '0x4689e3C91036437A46A6c8B62157F58210Ba67a7',
    pairName: 'SDT-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.sdt.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 144,
    address: '0x6Fab6CEdf26f9bF03448Fe835B674be1CFf0E8BB',
    pairName: 'agEUR-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.eura.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },

  {
    gid: 145,
    address: '0xd9e2a1a61B6E61b275cEc326465d417e52C1b95c',
    pairName: 'WETH-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 146,
    address: '0x0BaCc7a9717e70EA0DA5Ac075889Bd87d4C81197',
    pairName: 'WETH-USDT',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 147,
    address: '0x0d7c4b40018969f81750d0a164c3839a77353EFB',
    pairName: 'WETH-ARB',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.arb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 148,
    address: '0x7e928afb59f5dE9D2f4d162f754C6eB40c88aA8E',
    pairName: 'USDC-USDT',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdc.address,
    token1Address: arbitrumTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 149,
    address: '0xF5Fac36c2429e1Cf84D4aBACdB18477Ef32589c9',
    pairName: 'CAKE-WETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.cake.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 150,
    address: '0xD58522653D3F368D76d453Bc4C80CD7Fb36AC786',
    pairName: 'LVL-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.lvl.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 151,
    address: '0x5e3C3a063cc9A4AEB5310C7faDc2A98aEbDD245d',
    pairName: 'MGP-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.mgp.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 152,
    address: '0x9fFCA51D23Ac7F7df82da414865Ef1055E5aFCc3',
    pairName: 'ARB-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.arb.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 153,
    address: '0x81D1cc282e9a097115E59F67B9D81d4d1d00AC51',
    pairName: 'ARB-USDT',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.arb.address,
    token1Address: arbitrumTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 154,
    address: '0x54076C901D4fDF76C1fA1F77FAFC3FC1022ADBE5',
    pairName: 'WBTC-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 155,
    address: '0xd5d1F85E65Ce58a4782852F4A845b1d6Ca71F1a2',
    pairName: 'USDC-DAI',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdc.address,
    token1Address: arbitrumTokens.dai.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 156,
    address: '0x3fFCa56A99f477dd57A4a5d8799F4DA613C9956B',
    pairName: 'axlUSDC-USDT',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.axlUSDC.address,
    token1Address: arbitrumTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 157,
    address: '0xF3d0d1d3788FBd1F327149D30a4Eb7744861f05d',
    pairName: 'STG-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.stg.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 158,
    address: '0x7767FBfd90b557dc56554058E7c05c9fAa600F8F',
    pairName: 'STG-ARB',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.stg.address,
    token1Address: arbitrumTokens.arb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 159,
    address: '0x4573ef50dbb79769eF66B1b16dcDB60652884ba6',
    pairName: 'STG-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.stg.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 160,
    address: '0x1Cb2892038867aDFa78CCfc6C3fB89b1dA558243',
    pairName: 'PENDLE-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.pendle.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 161,
    address: '0x0CAA927059CB8db37CEefcB9e18c4FC7Efef9655',
    pairName: 'RDNT-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.rdnt.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 162,
    address: '0xF5BFda16f9E57F0B7a67C57b42407C33C31349B6',
    pairName: 'GMX-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.gmx.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 163,
    address: '0x46e3faBB9f963f84E9d23Ca2E332A3Ced59604a6',
    pairName: 'MAGIC-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.magic.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 164,
    address: '0x3ABBbBb4C254b9327F1a0580BdbCcb51B0b5Fb08',
    pairName: 'wstETH-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.wstETH.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 165,
    address: '0xC75908421566eA77A73B14D9cD0479C568f2B7A7',
    pairName: 'rETH-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.rETH.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 166,
    address: '0x0Ba3d55678C019B8101061855fe4Ea8D3ECE784f',
    pairName: 'LINK-WETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.link.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 167,
    address: '0xb901fc9D0D31C9A15DB69C616D31e19fF39b0df6',
    pairName: 'stEUR-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.stEUR.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 168,
    address: '0xC9057e7b625D293c0E308Ce344357E2d23174CE6',
    pairName: 'KUJI-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.kuji.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 169,
    address: '0xbA339883104bf9D1e894E1640E21E261bcacE6DD',
    pairName: 'KUJI-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.kuji.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 170,
    address: '0x98cffCe9A35132A42Da9582cCed57Db98C07A690',
    pairName: 'DMT-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.dmt.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 171,
    address: '0x6a23ec7A203F546d7D62FA667A652EC55197Ea6F',
    pairName: 'LINK-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.link.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 172,
    address: '0x278566F0ad52fbF36eb4Ba16d2171ed6e6e84e8a',
    pairName: 'EQB-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.eqb.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 173,
    address: '0xF5B4B24E5808DAA3fBeee11DF27a0994600356b4',
    pairName: 'VAI-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.vai.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 174,
    address: '0x7D3c51D707C8C63CB9f85cEC6E9F9FF0A5fb2735',
    pairName: 'VAI-USDC',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.vai.address,
    token1Address: bscTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 175,
    address: '0xB1D54d76E2cB9425Ec9c018538cc531440b55dbB',
    pairName: 'sdCAKE-CAKE',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.sdcake.address, bscTokens.cake.address],
  },
  {
    gid: 176,
    address: '0x060d8a5a7C03882e33AcA8FC304BabE869e21Ee9',
    pairName: 'GRAI-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.grai.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 177,
    address: '0x964f1c58aD7058F352fAEd049A4674c1990355AB',
    pairName: 'swETH-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.swETH.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 178,
    address: '0x3C2bf5C0Be7be4919fae1330b748E89f165259D7',
    pairName: 'axlUSDC-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.axlUSDC.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 179,
    address: '0xB5D01A6e99FdcDB6DB8D0A342C35036Adeb8FB48',
    pairName: 'KUJI-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.kuji.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 180,
    address: '0x2f74ad2c64be7DC07C9f51E9d338EcB7C1ee0B18',
    pairName: 'BONK-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.bonk.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 181,
    address: '0x1A1703Bf5f1Da9DB0f62d17e8c54e84Fd732D695',
    pairName: 'BONK-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.bonk.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 182,
    address: '0xDc9a574b9B341D4a98cE29005b614e1E27430E74',
    pairName: 'HZN-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.hzn.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 183,
    address: '0x719EBA3fB0799e215B6F82872F947418298e395e',
    pairName: 'BONK-USDT',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.bonk.address,
    token1Address: ethereumTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 184,
    address: '0xEd4D5317823Ff7BC8BB868C1612Bb270a8311179',
    pairName: 'INSP-USDT',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.insp.address,
    token1Address: ethereumTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 185,
    address: '0x7dF7c84f2f9DCef3c0813e539878B76B89a916F8',
    pairName: 'ID-USDC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.id.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 186,
    address: '0x80968a476E0235a026Fd1F33AAac5a3B7b76110F',
    pairName: 'AIOZ-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.aioz.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 187,
    address: '0x2Fdf9b25Df26e81598c09ef7482a82E2ec6eb68c',
    pairName: 'ACE-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ace.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 188,
    address: '0x14861FFC20f88E6D0Cf90da6BDc796Bb7f0B19E6',
    pairName: 'esRDNT-RDNT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.esRDNT.address,
    token1Address: bscTokens.rdnt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 189,
    address: '0x135Cd19cb3c15f7eB10FC21FF79e1259a65Fc958',
    pairName: 'AIOZ-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.aioz.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 190,
    address: '0xBC7766aE74f38f251683633d50Cc2C1CD14aF948',
    pairName: 'INSP-WETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.insp.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 191,
    address: '0x898fe77d1C083DD725004A12DF0bC55f6Dbdec2f',
    pairName: 'WBNB-USDV',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.bnb.address,
    token1Address: bscTokens.usdv.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 192,
    address: '0x589a5062e47202bB994cD354913733a14b54e8Dc',
    pairName: 'USDV-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdv.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 193,
    pairName: 'WETH-USDC',
    address: '0x291d9F9764c72C9BA6fF47b451a9f7885Ebf9977',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.weth.address,
    token1Address: zksyncTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 194,
    pairName: 'WETH-USDC',
    address: '0xFB467bedf483ef92D54b6615770eBEDD9F639a50',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.weth.address,
    token1Address: zksyncTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 195,
    pairName: 'USDC-USDT',
    address: '0x3832fB996C49792e71018f948f5bDdd987778424',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.usdc.address,
    token1Address: zksyncTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 196,
    pairName: 'CAKE-WETH',
    address: '0x3c11CAACc9FC70d9130792c39702C5F96cE68a93',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.cake.address,
    token1Address: zksyncTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 197,
    pairName: 'WETH-BTC',
    address: '0x9cB8b12cb0223e105155318B72AdddA15D588fB9',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.weth.address,
    token1Address: zksyncTokens.wbtc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 198,
    pairName: 'BUSD-USDC',
    address: '0x662cD659F91528FF27D7Cb6Ac25e6EBA11c4003C',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.busd.address,
    token1Address: zksyncTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 199,
    pairName: 'WETH-USDC',
    address: '0x7C0e7D6066aF191977a4483B445B5C06FC41ECd6',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.weth.address,
    token1Address: zksyncTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 200,
    pairName: 'BUSD-USDT',
    address: '0x38848d93a410446848CA55Fdc777Fe0B2C30B071',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.busd.address,
    token1Address: zksyncTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 201,
    pairName: 'rETH-ETH',
    address: '0x3693Ec2590e6bF8F221F61776dC9274AED7056D6',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.reth.address,
    token1Address: zksyncTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 202,
    pairName: 'USD+-USDC',
    address: '0x6a8Fc7e8186ddC572e149dFAa49CfAE1E571108b',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.usdPlus.address,
    token1Address: zksyncTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 203,
    pairName: 'RDP-BNB',
    address: '0xE297e1dA9a484E609D180C5B1fEfE4830df70eF1',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.rdp.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 204,
    pairName: 'sdCAKE-CAKE',
    address: '0xB1D54d76E2cB9425Ec9c018538cc531440b55dbB',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.sdcake.address, bscTokens.cake.address],
  },
  {
    gid: 205,
    pairName: 'sats-BNB',
    address: '0xcF0bA206D688Fc9AE8A268F8b9077F4307600895',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.sats.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 206,
    pairName: 'ordi-BNB',
    address: '0x442EFB99C2727c6000727C3cf672e3d77C9cDEaD',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ordi.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 207,
    pairName: 'MUBI-BNB',
    address: '0x961C19a53e5EA0e43B8F9020553F94623Ce475B6',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.mubi.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 208,
    pairName: 'NFP-USDT',
    address: '0x4b36782B47a28Ee003013E7C56174CB5812F418B',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.nfp.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 209,
    pairName: 'ordi-BNB',
    address: '0x1be0A017CDA061501b9dd460EBFD16f0971Eea8e',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ordi.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 210,
    pairName: 'MUBI-BNB',
    address: '0x57dBE41582591e8a420cc80FB669F953d137a571',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.mubi.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 211,
    pairName: 'WETH-USDT',
    address: '0x6Ff6B5c5957606220C6Cd0422499c9c1224c399b',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.weth.address,
    token1Address: zksyncTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 212,
    pairName: 'sdCAKE-CAKE',
    address: '0x8A876Ca851063e0252654CA6368a5B2280f51c32',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.sdcake.address,
    token1Address: bscTokens.cake.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 213,
    pairName: 'PNP-BNB',
    address: '0x88240a2CA0Af5DD3b181975b9985274274CB3685',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.pnp.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 214,
    pairName: 'mCAKE-CAKE',
    address: '0xb9dC6396AcFFD24E0f69Dfd3231fDaeB31514D02',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.mcake.address, bscTokens.cake.address],
  },
  {
    gid: 215,
    pairName: 'WETH-USDC',
    address: '0xD43b9dCbB61e6ccFbCFef9f21e1BB5064F1CB33f',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 216,
    pairName: 'WETH-USDT',
    address: '0x4A080D9488cE2C8258185d78852275D6d3c2820c',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.usdt.address,
    token1Address: polygonZkEvmTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 217,
    pairName: 'USDT-USDC',
    address: '0xca06375be938a2d6eF311dfaFab7E326d55D23Cc',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.usdt.address,
    token1Address: polygonZkEvmTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 218,
    pairName: 'WETH-MATIC',
    address: '0xaE30fcdEE41dC9eC265e841D8185d055B87d1B7a',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.matic.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 219,
    pairName: 'GRAI-USDC',
    address: '0x39aCc7cf02af19A1eB0e3628bA0F5C48f44beBF3',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.usdc.address,
    token1Address: polygonZkEvmTokens.grai.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 220,
    pairName: 'WETH-BTC',
    address: '0xf1e501f74Ed9dc619be53Fddb698c94AbF9D56B6',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.wbtc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 221,
    pairName: 'swETH-ETH',
    address: '0x6db0f81Db2C3B2A85a802d511577d8522D0D8C14',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.swETH.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 222,
    pairName: 'lisUSD-USDT',
    address: '0xB2Aa63f363196caba3154D4187949283F085a488',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.hay.address, bscTokens.usdt.address],
  },
  {
    gid: 223,
    pairName: 'USDT-CGPT',
    address: '0x1E8D7AB96323953faC85ab687f5D38b45344e62d',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.cgpt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 224,
    pairName: 'wROSE-USDT',
    address: '0x5B56eea565dad4E72B8ea6a6567834E144fb7a93',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.wrose.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 225,
    pairName: 'INSP-BNB',
    address: '0x06aC8EE32BCdcE6bF2eA82D9Bfb932a84D45BFcb',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.insp.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 226,
    pairName: 'FDUSD-BTC',
    address: '0x6AA9c4eDA3BF8ac038aD5C243133d6D25aA9cc73',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.fdusd.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 227,
    pairName: 'FDUSD-ETH',
    address: '0x63393D036FEf3B187ca613a8E5D917697ade2cD5',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.fdusd.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 228,
    pairName: 'FDUSD-CAKE',
    address: '0xA62439A42fbeC9827a0B0452A5fD619d517F2D83',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.fdusd.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 229,
    pairName: 'FDUSD-BNB',
    address: '0x0f3aDA81A32D8a7233fe9B1D04f0A8231AdB98F0',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.bnb.address,
    token1Address: bscTokens.fdusd.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 230,
    pairName: 'FDUSD-USDC',
    address: '0xf66A930Ed3b004BA16ee11b3A9B142EAf2259b0d',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdc.address,
    token1Address: bscTokens.fdusd.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 231,
    pairName: 'XAI-WETH',
    address: '0xf0B860d338E8B5199606322653B83A166d96E417',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.xai.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 232,
    pairName: 'USDT+-USD+',
    address: '0xb9c2d906f94b27bC403Ab76B611D2C4490c2ae3F',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdtplus.address,
    token1Address: arbitrumTokens.usdplus.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 233,
    pairName: 'USD+-ETH+',
    address: '0x06c75011479E47280e8B7E72E9e0315C8b3A634d',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.ethplus.address,
    token1Address: arbitrumTokens.usdplus.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 234,
    pairName: 'USD+-USDC.e',
    address: '0xd01075f7314a6436e8B74fc18069848229D0c555',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdplus.address,
    token1Address: arbitrumTokens.usdce.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 235,
    pairName: 'WETH-ETH+',
    address: '0xdAA80a051E22A7f7b0cfC33Aa29572fbDE65183E',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.ethplus.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 236,
    pairName: 'CKP-BNB',
    address: '0x8b626Acfb32CDad0d2F3b493Eb9928BbA1BbBcCa',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ckp.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 237,
    pairName: 'IRL-USDT',
    address: '0xce44eBE42ABc496eACBd6582b6e14f3937116B23',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.irl.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 238,
    pairName: 'ORDS-WETH',
    address: '0xBEA29eE3bb5F025D3C7e107883cD002c420b389d',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.ords.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 239,
    pairName: 'OSAK-WETH',
    address: '0xBc1a2e1B745497D214B99F359369d304D0aCf935',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.osak.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 240,
    pairName: 'MGP-BNB',
    address: '0x2b3DBbA2D1F5158c7BA4b645B7ea187F7F1763af',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.wbnb.address,
    token1Address: bscTokens.mgp.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 241,
    pairName: 'RDP-BNB',
    address: '0xc9B415b8331e1Fb0d2f3442Ac8413E279304090f',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.rdp.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 242,
    pairName: 'PNP-BNB',
    address: '0x1C5bD1B4A4Fc05cC0Fb1a0f61136512744Ca4F34',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.pnp.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 243,
    pairName: 'CKP-mCAKE',
    address: '0xdb92AD18eD18752a194b9D831413B09976B34AE1',
    chainId: ChainId.BSC,
    type: GaugeType.V2,
    token0Address: bscTokens.ckp.address,
    token1Address: bscTokens.mcake.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 244,
    pairName: 'HUAHUA-USDT',
    address: '0xE08078C1daACef415b2653e7256A70002F41Eeb6',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.huahua.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 245,
    pairName: 'ETH-USDC',
    address: '0xd5539D0360438a66661148c633A9F0965E482845',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.usdc.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 246,
    pairName: 'USDT-USDC',
    address: '0x6a72F4F191720c411Cd1fF6A5EA8DeDEC3A64771',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.usdc.address,
    token1Address: lineaTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 247,
    pairName: 'BTC-ETH',
    address: '0xbD3bc396C9393e63bBc935786Dd120B17F58Df4c',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.wbtc.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 248,
    pairName: 'USDC-DAI',
    address: '0xA48E0630B7b9dCb250112143C9D0fe47d26CB1e4',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.usdc.address,
    token1Address: lineaTokens.dai.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 249,
    pairName: 'USDC-axlUSDC',
    address: '0x85164B6d8a74bA481AB6D02D2C4e779ECCBAF982',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.usdc.address,
    token1Address: lineaTokens.axlusdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 250,
    pairName: 'wstETH-ETH',
    address: '0x3f63a467C54c96538bD36A7DF1b9E7C4719DcaC9',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.wstETH.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 251,
    pairName: 'AI-BNB',
    address: '0x85Bf2332c9033624B5d6f2607D8f07f22Bc86345',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.anyInu.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 252,
    pairName: 'wROSE-USDT',
    address: '0x9dD70024FF211bA754c089CDee47BdeDc17C4CBe',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.wrose.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 253,
    pairName: 'wstETH-ETH',
    address: '0x5631fE6d29E3CB717517DA05A9970e499DEF5e31',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.weth.address,
    token1Address: zksyncTokens.wstETH.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 254,
    pairName: 'OVN-USD+',
    address: '0x714D48cb99b87F274B33A89fBb16EaD191B40b6C',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.ovn.address,
    token1Address: arbitrumTokens.usdplus.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 255,
    pairName: 'OVN-USDT',
    address: '0xb624FE83Dfc393aAB921EbBc2Bf38Fdd238F2380',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ovn.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 256,
    pairName: 'MANTA-BNB',
    address: '0x93bA6af467c7B46B44D5C0BB300270c732510C9C',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.manta.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 257,
    pairName: 'USDC-wstETH',
    address: '0x16B3Bbd346eB864eD28B99cf89dDe59aD31A034F',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.usdc.address,
    token1Address: zksyncTokens.wstETH.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 258,
    pairName: 'pxETH-ETH',
    address: '0x3733493eC5D2C181Dcd7C54ed100641c0f07BB0e',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.pxETH.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 259,
    pairName: 'USDC.e-USDC',
    address: '0x849c0ae884bFDc14DDdeB7Cae95494f368414855',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.usdce.address,
    token1Address: polygonZkEvmTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },

  {
    gid: 260,
    pairName: 'WETH-WBNB',
    address: '0x753bA05488Cac9B3f7D59Ff7D3f13F31bB5eDf22',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },

  {
    gid: 261,
    pairName: 'mPENDLE-PENDLE',
    address: '0x183F325b33d190597D80d1B46D865d0250fD9BF2',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.mpendle.address, bscTokens.pendle.address],
  },
  {
    gid: 262,
    pairName: 'mDLP-DLP',
    address: '0xA2915ae3bc8C6C03f59496B6Dd26aa6a4335b788',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.mdlp.address, bscTokens.dlp.address],
  },
  {
    gid: 263,
    pairName: 'rBNB-BNB',
    address: '0x5b46fC8392cDEd6A91A79d85eFabFD51C7956cA0',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.rbnb.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 264,
    pairName: 'BNB-USDT',
    address: '0x172fcD41E0913e95784454622d1c3724f546f849',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 265,
    pairName: 'USD+-USDC',
    address: '0x721F37495cD70383B0A77Bf1eB8f97eef29498Bb',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdc.address,
    token1Address: arbitrumTokens.usdplus.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 266,
    pairName: 'USD+-USDT+',
    address: '0x8a06339Abd7499Af755DF585738ebf43D5D62B94',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdtplus.address,
    token1Address: arbitrumTokens.usdplus.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 267,
    pairName: 'weETH-WETH',
    address: '0x249CfCA66a4F6D02a12376D079E5d131423A6b7a',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.weETH.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 268,
    pairName: 'AITECH-USDT',
    address: '0x55458175c5F5D34B9bD01c81F172780ED4271b23',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.aitech.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 269,
    pairName: 'USDV-USD+',
    address: '0x35D85D531BE7159cB6f92E8B9CeaF04eC28c6ad9',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdv.address,
    token1Address: arbitrumTokens.usdplus.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 270,
    pairName: 'GTAI-USDT',
    address: '0xb24cd29e32FaCDDf9e73831d5cD1FFcd1e535423',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.gtai.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 271,
    pairName: 'SOL-BNB',
    address: '0x1E4600929Edf7F36B4A7eAc4C7571057D82a246c',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.sol.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 272,
    pairName: 'SOL-USDT',
    address: '0x9F5a0AD81Fe7fD5dFb84EE7A0CFb83967359BD90',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.sol.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 273,
    pairName: 'USDT-DMAIL',
    address: '0x4167f229a0323F480518b61CB35eD4d6a0C5EA27',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.dmail.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 274,
    pairName: 'WETH-USDbC',
    address: '0xe58b73fF901325b8b2056B29712C50237242F520',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.usdbc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 275,
    pairName: 'cbETH-WETH',
    address: '0xc0efC182479319ff258EcA420e2647cD82D3790c',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.cbETH.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 276,
    pairName: 'DAI-USDbC',
    address: '0xe4eFf19c7AcE186ba39fD3eD639B2D34171f7efF',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.dai.address,
    token1Address: baseTokens.usdbc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 277,
    pairName: 'WETH-DAI',
    address: '0x9BFa331679b307Cf358438F45f6413a205FD3EBf',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.dai.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 278,
    pairName: 'WETH-USDC',
    address: '0xB775272E537cc670C65DC852908aD47015244EaF',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 279,
    pairName: 'USDC-USDbC',
    address: '0x29Ed55B18Af0Add137952CB3E29FB77B32fCE426',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.usdbc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 280,
    pairName: 'DAI-USDC',
    address: '0x345825A980BD94e1480bC4f20FE4e3DAE2F23Dd3',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.dai.address,
    token1Address: baseTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 281,
    pairName: 'tBTC-WETH',
    address: '0x0D486753B99b1e0548d3505D8B797c673b58Cad3',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.tbtc.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 282,
    pairName: 'WETH-USDbC',
    address: '0xF6C0A374a483101e04EF5F7Ac9Bd15d9142BAC95',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.usdbc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 283,
    pairName: 'WETH-USDC',
    address: '0x72AB388E2E2F6FaceF59E3C3FA2C4E29011c2D38',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 284,
    pairName: 'USDbC-axlUSDC',
    address: '0x25DEe2707979055245A18AE6a415bb7b1435Eb06',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.usdbc.address,
    token1Address: baseTokens.axlusdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 285,
    pairName: 'wstETH-ETH',
    address: '0x90375306810C6E8B2efa8294835C78B499D7c691',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.wstETH.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 286,
    pairName: 'WETH-USDT',
    address: '0xc014414696F332C96C471634620344143325D2C0',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.usdt.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 287,
    pairName: 'DAI-USDT',
    address: '0x3846400eEe47B29B5A6742620a29128A24921659',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.usdt.address,
    token1Address: zksyncTokens.dai.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 288,
    pairName: 'DAI-USDC',
    address: '0x374D5C205742AEbE2D9fe9B9741EFA7E12082234',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.usdc.address,
    token1Address: zksyncTokens.dai.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 289,
    pairName: 'wstETH-rETH',
    address: '0xB3348f622e2d0F9053E08DA178445B80016c18Fb',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.reth.address,
    token1Address: zksyncTokens.wstETH.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 290,
    pairName: 'OSAK-WBNB',
    address: '0xe0D4f397935A6346c8E2cd2aBa4ab5Ff26532352',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.osak.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 291,
    pairName: 'wstETH-ETH',
    address: '0x3752BDb9215A2C0609d71407b50A528e1C6ECaBD',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.wstETH.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 292,
    pairName: 'rETH-ETH',
    address: '0x2e780D6666C2C44754a0cA7A8e2Ed97506B47751',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.reth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 293,
    pairName: 'rsETH-ETH',
    address: '0x435564Fb7E82daB83B2733D6Bc1fDB8B5a732DE8',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.rsETH.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 294,
    pairName: 'wstETH-rETH',
    address: '0xf142d852c9c805e419399c9B3Ce2A8485F950BBf',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.wstETH.address,
    token1Address: polygonZkEvmTokens.reth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 295,
    pairName: 'ETH-USDC',
    address: '0x586733678b9aC9Da43dD7CB83bbB41d23677Dfc3',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.usdc.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 296,
    pairName: 'BTC-ETH',
    address: '0x5AFda31027C3E6A03c77a113FFC031B564AbbF05',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.wbtc.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 297,
    pairName: 'WETH-USDC',
    address: '0x9f37552b87b68E7F169c442D595c1Be7A0F03b92',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 298,
    pairName: 'WETH-BTC',
    address: '0xb5d9E1622BFA6Efb3FB50c0bDc6a0EE2b2d046fA',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.wbtc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 299,
    pairName: 'WETH-MATIC',
    address: '0x7a816241EdaF060e33b774D6D3D6398485dFf9AF',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.matic.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 300,
    pairName: 'WETH-USDT',
    address: '0x8126D76CE6d80C93E2E03c9E39a676Aea542c01D',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.usdt.address,
    token1Address: zksyncTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 301,
    pairName: 'WETH-BTC',
    address: '0x9aFFdEe9004892624BFFebeB8EDBa4C980Fe6aCF',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.weth.address,
    token1Address: zksyncTokens.wbtc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 302,
    pairName: 'cbETH-WETH',
    address: '0x257FCbAE4Ac6B26A02E4FC5e1a11e4174B5ce395',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.cbETH.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 303,
    pairName: 'mswETH-swETH',
    address: '0x6177811663A60Ac211566bE5873c5Ed441D9E948',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.mswETH.address,
    token1Address: ethereumTokens.swETH.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 304,
    pairName: 'mstETH-wstETH',
    address: '0x350d6d813Be7B64681f91F16A98Ef360Bd42b66b',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.mstETH.address,
    token1Address: ethereumTokens.wstETH.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 305,
    pairName: 'pxETH-ETH',
    address: '0x8Da885A16bAceA79eD681b235Be1E1C45d3E3049',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.pxETH.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 306,
    pairName: 'WETH.e-WETH',
    address: '0x424594bD8B08E3F0c0e282B11Cc5817ae4eC47bf',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.wethe.address,
    token1Address: zksyncTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 307,
    pairName: 'CAKE-USDC',
    address: '0xb4BAB40e5a869eF1b5ff440a170A57d9feb228e9',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.cake.address,
    token1Address: polygonZkEvmTokens.usdc.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 308,
    pairName: 'CAKE-ETH',
    address: '0x3Fa1c450f3842C1252e4cB443e3F435b41D6f472',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.cake.address,
    token1Address: polygonZkEvmTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 309,
    pairName: 'BNX-USDT',
    address: '0xfab21Cb9467e9BaDd22A2dE57BCDE5F53D925973',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.bnx.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 310,
    pairName: 'CAKE-ETH',
    address: '0xE817A59F8A030544Ff65F47536abA272F6d63059',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.cake.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 311,
    pairName: 'CAKE-ETH',
    address: '0x03C33a2fC0D444a5B61E573f9e1A285357a694fc',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.cake.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 312,
    pairName: 'FET-BNB',
    address: '0x2cBfA271CcE515FEd6e49b406Df4A77ec952a952',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.fet.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 313,
    pairName: 'USDC-BNB',
    address: '0x81A9b5F18179cE2bf8f001b8a634Db80771F1824',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdc.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 314,
    pairName: 'USDC-BNB',
    address: '0xf2688Fb5B81049DFB7703aDa5e770543770612C4',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdc.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 315,
    pairName: 'pxETH-ETH',
    address: '0xf9Adc7F9E10eAA8AfD6A3Ea2b419d9497fa192c3',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.pxETH.address,
    token1Address: bscTokens.eth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 316,
    pairName: 'XRGB-BNB',
    address: '0x9a5936bab8f9ae1D89DA5fa95C484F7aD597C8C0',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.xrgb.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 317,
    pairName: 'GRAI-USDC',
    address: '0x6c94C8651063C1aA67f9C15284235a3D13cD383C',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.grai.address,
    token1Address: zksyncTokens.usdc.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 318,
    pairName: 'GRAI-USDC',
    address: '0xA9e8fb4462A4140a2cec9E4fcdFa592AA6B786Ed',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    token0Address: zksyncTokens.grai.address,
    token1Address: zksyncTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 319,
    pairName: 'XRGB-ETH',
    address: '0x7E9570FD7B4f1362aB924dfDE29096e0B484E009',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.xrgb.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 320,
    pairName: 'VIA-USDT',
    address: '0x0125ac49a5C06CCfd0819e6E20CCBE09bDE35140',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.octavia.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 321,
    pairName: 'TRUMP-BNB',
    address: '0x032AB93B734BEf57D98552EB56839F0995D4FB28',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.trump.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 322,
    pairName: 'FIL-BNB',
    address: '0x505D091fd01b3D4340827A3737c950f030CdD9A7',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.fil.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 323,
    pairName: 'ETH-ezETH',
    address: '0xb5Bf551Df2C078890809Ad295c91ec5aBC23e620',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.ezETH.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 324,
    pairName: 'PEPE-WBNB',
    address: '0xdD82975ab85E745c84e497FD75ba409Ec02d4739',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.pepe.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 325,
    pairName: 'USDT-DECHAT',
    address: '0x1Cf977828364c597a4a80779a3a5eb8fB673A845',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.chat.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 326,
    pairName: 'WETH-USDC',
    address: '0x7fCDC35463E3770c2fB992716Cd070B63540b947',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 327,
    pairName: 'WETH-USDT',
    address: '0x389938CF14Be379217570D8e4619E51fBDafaa21',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 328,
    pairName: 'USDT-WETH',
    address: '0x1947B87d35E9f1cd53CEDe1aD6F7be44C12212B8',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.usdt.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 329,
    pairName: 'WETH-wstETH',
    address: '0xBd59a718E60bd868123C6E949c9fd97185EFbDB7',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.wstETH.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 330,
    pairName: 'WETH-USDC',
    address: '0x1445F32D1A74872bA41f3D8cF4022E9996120b31',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.usdc.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 331,
    pairName: 'WETH-USDT',
    address: '0xaCDb27b266142223e1e676841C1E809255Fc6d07',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 332,
    pairName: 'GRAI-USDC.E',
    address: '0x160f3d3af6A2ECe5274AfD0925137e0387BAA5F2',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.usdce.address,
    token1Address: polygonZkEvmTokens.grai.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 333,
    pairName: 'EGO-USDT',
    address: '0xEDB6d7d456A570f790185aF9f6B99020Cc578Ed0',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.ego.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 334,
    pairName: 'ezETH-ETH',
    address: '0xfDe733b5DE5B5a06C68353e01E4c1D3415C89560',
    chainId: ChainId.LINEA,
    type: GaugeType.V3,
    token0Address: lineaTokens.ezETH.address,
    token1Address: lineaTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 335,
    pairName: 'WETH-USDT',
    address: '0xa20bc2656e1b310b6CA711c012D7a36e15712Cda',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdt.address,
  },
  {
    gid: 336,
    pairName: 'WETH-ARB',
    address: '0xDce0e85Ed514626d698619Be2135B0f3E16E8E06',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.arb.address,
  },
  {
    gid: 337,
    pairName: 'WBTC-WETH',
    address: '0xC6BACE1E2976a7448803F0D686c1dCC4BA8319d9',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 338,
    pairName: 'ARB-USDC',
    address: '0xea42d38AfeAdbA743123e11f012FC017A7534e68',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.arb.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 339,
    pairName: 'WETH-USDC',
    address: '0xdf3BB5c34B2502c2556f70CaF2B215477b4aCC93',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 340,
    pairName: 'RDNT-WETH',
    address: '0xD6d29209C256aD605cAB2a0aa3A7e68BA25bD9E6',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.rdnt.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 341,
    pairName: 'WETH-GMX',
    address: '0xee58dC42bC4E80331553CF6c4eb11efA54fFecDB',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.gmx.address,
  },
  {
    gid: 342,
    pairName: 'WETH-LINK',
    address: '0x2167812429122441eaF7ADC50Ac96BCA1A39aA7D',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.link.address,
  },
  {
    gid: 343,
    pairName: 'WETH-swETH',
    address: '0xD0dCEf57Dd0F196dD549a3C56332166C2f2ded4b',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.swETH.address,
  },
  {
    gid: 344,
    pairName: 'wstETH-ETHx',
    address: '0x97e4763B55D06230f7FAE335805edf53fC2E3275',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.wstETH.address,
    token1Address: ethereumTokens.ethx.address,
  },
  {
    gid: 345,
    pairName: 'USDC-ETHx',
    address: '0x1b6cEecc5FC0B26E9b85695F649c3A45c18dd46c',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.usdc.address,
    token1Address: ethereumTokens.ethx.address,
  },
  {
    gid: 346,
    pairName: 'mPENDLE-PENDLE',
    address: '0x73ed25e04Aa673ddf7411441098fC5ae19976CE0',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.StableSwap,
    tokenAddresses: [arbitrumTokens.mpendle.address, arbitrumTokens.pendle.address],
    killed: true,
  },
  {
    gid: 347,
    pairName: 'mdLP-dLP',
    address: '0xd0f0be815a76eFE677c92b07b39a5e972BAf22bD',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.StableSwap,
    tokenAddresses: [arbitrumTokens.mdlp.address, arbitrumTokens.dlp.address],
    killed: true,
  },
  {
    gid: 348,
    pairName: 'CAKE-WETH',
    address: '0xAC98137c3dbE929aE354dDCF50E73Bf59d88dB00',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.cake.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 349,
    pairName: 'USDC.E-GRAI',
    address: '0x5Eb74D624b60B3A9336a37509AF62f6906975264',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.ALM,
    token0Address: polygonZkEvmTokens.usdce.address,
    token1Address: polygonZkEvmTokens.grai.address,
  },
  {
    gid: 350,
    pairName: 'MATIC-GRAI',
    address: '0x03B832F38878DC9478Ed8127Cb46C7c094a94941',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.ALM,
    token0Address: polygonZkEvmTokens.matic.address,
    token1Address: polygonZkEvmTokens.grai.address,
  },
  {
    gid: 351,
    pairName: 'rBNB-WBNB',
    address: '0xB22e845415C0B28854DC59D8FF14d12Cb757108B',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.wbnb.address,
    token1Address: bscTokens.rbnb.address,
  },
  {
    gid: 352,
    pairName: 'WBNB-rBNB',
    address: '0xA9bC9d27B040944388F1D43D7Ed65584aa692b17',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.wbnb.address,
    token1Address: bscTokens.rbnb.address,
  },
  {
    gid: 353,
    pairName: 'rETH-WETH',
    address: '0x581046c928cd7e892654A606Ce66F98b026DC793',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.rETH.address,
    token1Address: ethereumTokens.weth.address,
  },
  {
    gid: 354,
    pairName: 'USDT-CAKE',
    address: '0x0Def612e7a7b51ca7Ee38f7905dA809BD3491268',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.usdt.address,
  },
  {
    gid: 355,
    pairName: 'USDT-WBNB',
    address: '0x553A31Bb23218a10FF1fFE6492f0D3C71fb88D24',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 356,
    pairName: 'WBNB-CAKE',
    address: '0x8f22d7B80446F1dc4C5eb47F2038e17CA0306e0D',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 357,
    pairName: 'USDT-BTCB',
    address: '0x585374f0502B8E2814e5A3eB16B06DB964610b77',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.btcb.address,
  },
  {
    gid: 358,
    pairName: 'SDT-BNB',
    address: '0x71b948B361ad69A233a208190C9ac9c5B130d095',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.sdt.address,
    token1Address: bscTokens.bnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 359,
    pairName: 'WETH-rswETH',
    address: '0x2689eF3EC44f417d22c862F16F3D5e71E6287086',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.rswETH.address,
  },
  {
    gid: 360,
    pairName: 'BNB-USDT',
    address: '0xc4f981189558682F15F60513158B699354B30204',
    chainId: ChainId.OPBNB,
    type: GaugeType.V3,
    token0Address: opBnbTokens.wbnb.address,
    token1Address: opBnbTokens.usdt.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 361,
    pairName: 'FDUSD-USDT',
    address: '0xD9004241D34392e9Ae0C84d5aCDF76941a27D8D1',
    chainId: ChainId.OPBNB,
    type: GaugeType.V3,
    token0Address: opBnbTokens.fdusd.address,
    token1Address: opBnbTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 362,
    pairName: 'ETH-BNB',
    address: '0xFf00F4E09820dbbe8582F771800732DaE7F002bD',
    chainId: ChainId.OPBNB,
    type: GaugeType.V3,
    token0Address: opBnbTokens.wbnb.address,
    token1Address: opBnbTokens.eth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 363,
    pairName: 'NMT-BNB',
    address: '0x2450f6B521567F0eDc8a95bF3CF5C452E4f9cC67',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.nmt.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 364,
    pairName: 'NMT-USDC',
    address: '0x93c041eDa52849753066B6D729e5a15C19766A69',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.nmt.address,
    token1Address: ethereumTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 365,
    pairName: 'ETH-rswETH',
    address: '0x7B94A5622035207d3f527d236d47B7714Ee0acBa',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.rswETH.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 366,
    pairName: 'USDT-XCAD',
    address: '0x91C348015Eb3Bb32f5AaE59a594c727365816811',
    chainId: ChainId.OPBNB,
    type: GaugeType.V3,
    token0Address: opBnbTokens.usdt.address,
    token1Address: opBnbTokens.xcad.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 367,
    pairName: 'MASA-BNB',
    address: '0x6AFe0F2b39B98857e49af22cEf4e4525551B46ac',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.masa.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 368,
    pairName: 'MASA-ETH',
    address: '0x021E6270091a926D08d43681978d65F9A9b024ca',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.masa.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 369,
    pairName: 'CAKE-WBNB',
    address: '0x1f36E4aEdA53F7fac687a8D3Ee97D6A4909DE512',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 370,
    pairName: 'USDT-WBNB',
    address: '0x27c42da0cCC1b394426858667d7ec9f0b0373187',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 371,
    pairName: 'USDC-USDbC',
    address: '0x3875Bf4a7DB5aF64f9A9659ec371678743598594',
    chainId: ChainId.BASE,
    type: GaugeType.ALM,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.usdbc.address,
  },
  {
    gid: 372,
    pairName: 'USDC-WETH',
    address: '0x44e4a7f3de18C6E843bFe8754C23aA4e67e6A433',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.ALM,
    token0Address: zksyncTokens.usdc.address,
    token1Address: zksyncTokens.weth.address,
  },
  {
    gid: 373,
    pairName: 'CAKE-BTCB',
    address: '0x4005A85F68f307cD97e23F7710162E1eBE6139Ac',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.btcb.address,
  },
  {
    gid: 374,
    pairName: 'CAKE-ETH',
    address: '0x17A39482e7f42ebB0f65d2978dcB1423E399D07a',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.eth.address,
  },
  {
    gid: 375,
    pairName: 'ETH-USDC',
    address: '0x57424fF60D0a23cB13CAf995c39E41FDB7c4fc1f',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.usdc.address,
  },
  {
    gid: 376,
    pairName: 'USDT-USDC',
    address: '0xFEB0819A3d00EACf1D8F593D2538C33d34b76274',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.usdc.address,
  },
  {
    gid: 377,
    pairName: 'USDT-WBNB',
    address: '0xB99F1Ce0f1C95422913FAF5b1ea980BbC580c14a',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 378,
    pairName: 'mPENDLE-PENDLE',
    address: '0x1A2329546f11e4fE55b853D98Bba2c4678E3105A',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.StableSwap,
    tokenAddresses: [arbitrumTokens.mpendle.address, arbitrumTokens.pendle.address],
  },
  {
    gid: 379,
    pairName: 'mdLP-dLP',
    address: '0x0db5e247ab73FBaE16d9301f2977f974EC0AA336',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.StableSwap,
    tokenAddresses: [arbitrumTokens.mdlp.address, arbitrumTokens.dlp.address],
  },
  {
    gid: 380,
    pairName: 'USDT-WBNB',
    address: '0xfCCa9B42A366d9493E7c0a7eC4aD96E1B4204dfd',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 381,
    pairName: 'CAKE-WBNB',
    address: '0x5db61A5f05580Cf620a9d0f9266E7432811DC309',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.cake.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 382,
    pairName: 'USDT-USDC',
    address: '0x52086FE16AC8D95537d8a1a00d57eBFBA7f92289',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.usdc.address,
    token1Address: ethereumTokens.usdt.address,
  },
  {
    gid: 383,
    pairName: 'WETH-USDT',
    address: '0xFf9488D98d03Eaffe4B56B462c037769101b1b9c',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.usdt.address,
  },
  {
    gid: 384,
    pairName: 'WETH-USDC',
    address: '0xf670f7fA7E55eD9816fBE2685aEB1B4E963F923D',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.ALM,
    token0Address: zksyncTokens.usdc.address,
    token1Address: zksyncTokens.weth.address,
  },
  {
    gid: 385,
    pairName: 'USDT-WETH',
    address: '0x5b8471581A1c9167202C9E2406c147C55e706e0a',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.ALM,
    token0Address: zksyncTokens.usdt.address,
    token1Address: zksyncTokens.weth.address,
  },
  {
    gid: 386,
    pairName: 'WETH-USDT',
    address: '0x352984DBF1EF87C0840bD48043021fFeC2fcAcED',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.ALM,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.usdt.address,
  },
  {
    gid: 387,
    pairName: 'CKP-WBNB',
    address: '0xBc5Bbf09F1d20724E083E75B92E48073172576f7',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.ckp.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 388,
    pairName: 'WETH-MGP',
    address: '0x7B89B8c243bd56301B0B749a16B071A8285203c8',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.mgp.address,
  },
  {
    gid: 389,
    pairName: 'mwBETH-wBETH',
    address: '0x4cBEa76B4A1c42C356B4c52B0314A98313fFE9df',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.mwbeth.address, bscTokens.wbeth.address],
  },
  {
    gid: 390,
    pairName: 'WBTC-ETH',
    address: '0x4bfc22A4dA7f31F8a912a79A7e44a822398b4390',
    chainId: ChainId.ARBITRUM_ONE,
    feeTier: FeeAmount.LOWEST,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 391,
    pairName: 'WETH-ARB',
    address: '0x11d53EC50bc8F54B9357fbFe2A7dE034FC00f8b3',
    chainId: ChainId.ARBITRUM_ONE,
    feeTier: FeeAmount.LOWEST,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.arb.address,
  },
  {
    gid: 392,
    pairName: 'ARB-USDC',
    address: '0x93CCe474015007B38dA0eceA96671EE4dc3d40Ad',
    chainId: ChainId.ARBITRUM_ONE,
    feeTier: FeeAmount.LOWEST,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.arb.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 393,
    pairName: 'ARB-USDT',
    address: '0xB19005B42E2Dcc65FB6A5598db329EDFe365A0b2',
    chainId: ChainId.ARBITRUM_ONE,
    feeTier: FeeAmount.LOWEST,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.arb.address,
    token1Address: arbitrumTokens.usdt.address,
  },
  {
    gid: 394,
    pairName: 'WETH-USDT',
    address: '0x337AF062CE32bB423010415196E315c4154D36C3',
    chainId: ChainId.POLYGON_ZKEVM,
    feeTier: FeeAmount.LOWEST,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.weth.address,
    token1Address: polygonZkEvmTokens.usdt.address,
  },
  {
    gid: 395,
    pairName: 'DAI-USDC',
    address: '0x9Bc342259cCEdA0487E70b71A3161F002a95F0e8',
    chainId: ChainId.POLYGON_ZKEVM,
    feeTier: FeeAmount.LOWEST,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.dai.address,
    token1Address: polygonZkEvmTokens.usdc.address,
  },
  {
    gid: 396,
    pairName: 'FOXY-ETH',
    address: '0x96F8d07B34e15c0f9338e79fD038F0f7A0C3a676',
    chainId: ChainId.LINEA,
    feeTier: FeeAmount.HIGH,
    type: GaugeType.V3,
    token0Address: lineaTokens.foxy.address,
    token1Address: lineaTokens.weth.address,
  },
  {
    gid: 397,
    pairName: 'SDT-WBNB',
    address: '0x52b59E3eAdc7C4ce8d3533020ca0Cd770E4eAbC3',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.sdt.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 398,
    pairName: 'USDT+-USD+',
    address: '0x68C56801e1919A3bA248968630586833488f3B25',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: zksyncTokens.usdtPlus.address,
    token1Address: zksyncTokens.usdPlus.address,
  },
  {
    gid: 399,
    pairName: 'USDT-USDT+',
    address: '0x5bc25c641174e443ba618d86fC63639CD5288dF0',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: zksyncTokens.usdt.address,
    token1Address: zksyncTokens.usdtPlus.address,
  },
  {
    gid: 400,
    pairName: 'WBTC-USDC',
    address: '0x843aC8dc6D34AEB07a56812b8b36429eE46BDd07',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 401,
    pairName: 'SolvBTC-BTCB',
    address: '0x12197d7a4fE2d67F9f97ae64D82A44c24B7Ad407',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.solvbtc.address,
    token1Address: bscTokens.btcb.address,
  },
  {
    gid: 402,
    pairName: 'STONE-ETH',
    address: '0xb66cB4092277ee946d4FbA1D34f425329fEdd37e',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.stone.address,
    token1Address: bscTokens.eth.address,
  },
  {
    gid: 403,
    pairName: 'USDC.e-WETH',
    address: '0x07811284e36fDc45f65cd56FC7c6929855d6A0cc',
    chainId: ChainId.LINEA,
    type: GaugeType.ALM,
    token0Address: lineaTokens.usdc.address,
    token1Address: lineaTokens.weth.address,
  },
  {
    gid: 404,
    pairName: 'WBTC-WETH',
    address: '0x0F3CC3Ea42b989323e7c7e499b5B6A343eA55c18',
    chainId: ChainId.LINEA,
    type: GaugeType.ALM,
    token0Address: lineaTokens.wbtc.address,
    token1Address: lineaTokens.weth.address,
  },
  {
    gid: 405,
    pairName: 'USDC.e-USDT',
    address: '0xd4E10dd0c0e64C5F6EB134E7D2F2D43f82D8DC00',
    chainId: ChainId.LINEA,
    type: GaugeType.ALM,
    token0Address: lineaTokens.usdc.address,
    token1Address: lineaTokens.usdt.address,
  },
  {
    gid: 406,
    pairName: 'wstETH-WETH',
    address: '0x8FCc61e802c6356486e37d45b53D212af34Cc2ae',
    chainId: ChainId.LINEA,
    type: GaugeType.ALM,
    token0Address: lineaTokens.wstETH.address,
    token1Address: lineaTokens.weth.address,
  },
  {
    gid: 407,
    pairName: 'BNB-BUSD',
    address: '0x8F45b99BF65CDBF9bC0C0b4846D6a324d2DE5314',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: bscTokens.bnb.address,
    token1Address: bscTokens.busd.address,
  },
  {
    gid: 408,
    pairName: 'USDC-USDC.e',
    address: '0x3AEf05a8E7D7A83f5527edeD214e0b24A87d0991',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: zksyncTokens.usdcNative.address,
    token1Address: zksyncTokens.usdc.address,
  },
  {
    gid: 409,
    pairName: 'USDC-USDT',
    address: '0xD05eEf3792276E92bB051029DaDFc2Bf81121692',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: zksyncTokens.usdcNative.address,
    token1Address: zksyncTokens.usdt.address,
  },
  {
    gid: 410,
    pairName: 'USDC-ETH',
    address: '0xEE53B979d0C4B2A40279287965b918BD2a4B760D',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: zksyncTokens.usdcNative.address,
    token1Address: zksyncTokens.weth.address,
  },
  {
    gid: 411,
    pairName: 'FURY-USDT',
    address: '0xa62FdD52Ae4c9Ca11d6C4e328E8F6A9Db218E452',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.fury.address,
    token1Address: bscTokens.usdt.address,
  },
  {
    gid: 412,
    pairName: 'MNT-mETH',
    address: '0x11b544C90EE643ca0256696F43b4512A4eD2D049',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: ethereumTokens.mnt.address,
    token1Address: ethereumTokens.mETH.address,
  },
  {
    gid: 413,
    pairName: 'WETH-mETH',
    address: '0x5bF7C2C688b058f3C89aa667e4698349D062c813',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.mETH.address,
    killed: true,
  },
  {
    gid: 414,
    pairName: 'WETH-mETH',
    address: '0xfcc3B8959d99f1154F0B1ac0252aa7e0F0617BCC',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.mETH.address,
  },
  {
    gid: 415,
    pairName: 'USD+-WETH',
    address: '0xe37304F7489ed253b2A46A1d9DabDcA3d311D22E',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdplus.address,
  },
  {
    gid: 416,
    pairName: 'USD+-ARB',
    address: '0xf92768916015b5eBd9fa54D6BA10dA5864e24914',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.arb.address,
    token1Address: arbitrumTokens.usdplus.address,
  },
  {
    gid: 417,
    pairName: 'USD+-USDC',
    address: '0xa1F9159e11aD48524c16C9bf10bf440815b03e6C',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usdc.address,
    token1Address: arbitrumTokens.usdplus.address,
  },
  {
    gid: 418,
    pairName: 'USDV-USDC',
    address: '0xb18eA0b56E46D4F890dC729425184fD1EF25dbFf',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usdv.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 419,
    pairName: 'USDV-USDT',
    address: '0x667ED8FCa4fc042B8425AA3547E3271d9E6E9bB4',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usdv.address,
    token1Address: arbitrumTokens.usdt.address,
  },
  {
    gid: 420,
    pairName: 'GRAI-ETH',
    address: '0xb0C6Be7b6aaB831092A3c7ef3Fe2Ab4F48B80b6A',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.grai.address,
  },
  {
    gid: 421,
    pairName: 'rsETH-ETH',
    address: '0x4B89179dAfe5c36AE611208B5590FEf9153b7daa',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.rsETH.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 422,
    pairName: 'ETHx-ETH',
    address: '0xe2d9b612446180334D56e10e82Dc78E3dEe814B0',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.ethX.address,
  },
  {
    gid: 423,
    pairName: 'ezETH-ETH',
    address: '0x1D2031Fa0dA6fb4bDA57E377C7273c22eab930D6',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.ezETH.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 424,
    pairName: 'weETH-ETH',
    address: '0x64dae6685725Dbd0a0e63fE522c9134d0EaA7258',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.weETH.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 425,
    pairName: 'axlUSDC-USDC.e',
    address: '0x5255a4115Bf7B98cc5cb037f24A8F12527c04e45',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.axlUSDC.address,
    token1Address: arbitrumTokens.usdce.address,
  },
  {
    gid: 426,
    pairName: 'FLY-USDC',
    address: '0x64bC7Ed8a3F827D47B0041430875A7778B7DB104',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.fly.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 427,
    pairName: 'USDV-WETH',
    address: '0x6fd1112CdFA0F53a698Fb4D6c6f7dCEc82e20B58',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.usdv.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 428,
    pairName: 'MAGIC-ETH',
    address: '0xbd536591E3D2FBa81Adb8Dff970a43A8A0d382EC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.magic.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 429,
    pairName: 'TIA.n-ETH',
    address: '0xfeb104864f3aD0C75E6B1bB4A0EDB3f89e562e6e',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.tia.address,
  },
  {
    gid: 430,
    pairName: 'WOO-ETH',
    address: '0x9BC7abdfaEfA730d63A904acCEC61DD9083AA638',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.woo.address,
  },
  {
    gid: 431,
    pairName: 'MIM-USDC',
    address: '0xB27fdE8d172bb146186Bac819Ceb12fE4FA9262A',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usdc.address,
    token1Address: arbitrumTokens.mim.address,
  },
  {
    gid: 432,
    pairName: 'ARB-USDC.e',
    address: '0x3a63E393726640B33A78A3cac5e57fa6f35B69Ed',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.arb.address,
    token1Address: arbitrumTokens.usdce.address,
  },
  {
    gid: 433,
    pairName: 'USDC-USDC.e',
    address: '0xE8BB56ee968333bf18f9CBbE8eEef350540F9607',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usdc.address,
    token1Address: arbitrumTokens.usdce.address,
  },
  {
    gid: 434,
    pairName: 'USDC.e-USDT',
    address: '0x858dff46915d3f81bb7fF06158B2806164ACB2A0',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usdt.address,
    token1Address: arbitrumTokens.usdce.address,
  },
  {
    gid: 435,
    pairName: 'WBTC-USDC',
    address: '0x5A17cbf5F866BDe11C28861a2742764Fac0Eba4B',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 436,
    pairName: 'WETH-USDC.e',
    address: '0x4fC7aC44DAe553464d92c049175133239A8705C1',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.usdce.address,
  },
  {
    gid: 437,
    pairName: 'FUSDC-USDC',
    address: '0x38437dBfd1D6a147A50CD0991A9581Ac8f996892',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.fusdc.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 438,
    pairName: 'TAIKO-ETH',
    address: '0x05dF8F9fCFf0b6a6FDE7166706a52693906C9936',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: ethereumTokens.taiko.address,
    token1Address: ethereumTokens.weth.address,
  },
  {
    gid: 439,
    pairName: 'USDT-axlSTARS',
    address: '0x0a3178B9C9FbCdE0fDFcFd805ce776eB3b8138e3',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.axlSTARS.address,
  },
  {
    gid: 440,
    pairName: 'USD+-WETH',
    address: '0x5b9FEB72588D2800892a00d2abB4ca9071df846e',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.usdPlus.address,
    token1Address: baseTokens.weth.address,
  },
  {
    gid: 441,
    pairName: 'wstETH-USD+',
    address: '0xa4846201E94D2a5399774926f760A36D52Ac22BF',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.wstETH.address,
    token1Address: baseTokens.usdPlus.address,
  },
  {
    gid: 442,
    pairName: 'cbETH-USD+',
    address: '0x40C91EBd1FA940A363989aC80a31B3a988dD649B',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.cbETH.address,
    token1Address: baseTokens.usdPlus.address,
  },
  {
    gid: 443,
    pairName: 'BRETT-USD+',
    address: '0x98Ee8cd99370Ab19F18Fb9033337995076867ee9',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.brett.address,
    token1Address: baseTokens.usdPlus.address,
  },
  {
    gid: 444,
    pairName: 'DEGEN-USD+',
    address: '0xdd5AC923f03a97FF9F0cfbFa0F5E155E46c3727d',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.degen.address,
    token1Address: baseTokens.usdPlus.address,
  },
  {
    gid: 445,
    pairName: 'AERO-USD+',
    address: '0x62996340a9bFEeE2A72bfAE8F21b8c0A5E692261',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.aero.address,
    token1Address: baseTokens.usdPlus.address,
  },
  {
    gid: 446,
    pairName: 'OVN-WETH',
    address: '0xcC7BfD85395042EE0cACe335E40b549b3d08Eb78',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: baseTokens.ovn.address,
    token1Address: baseTokens.weth.address,
  },
  {
    gid: 447,
    pairName: 'ZK-ETH',
    address: '0x96d33cF093857AC8C7a5f2183f405A0FCcC42444',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: zksyncTokens.zk.address,
    token1Address: zksyncTokens.weth.address,
  },
  {
    gid: 448,
    pairName: 'ZK-ETH',
    address: '0xC081eACC77c75CE1f39a43c04b53D90ADaD35fFd',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: zksyncTokens.zk.address,
    token1Address: zksyncTokens.weth.address,
  },
  {
    gid: 449,
    pairName: 'ZK-USDC.e',
    address: '0x3bF35ac7BF2E4aaF98e007c9C3e0d214562A3DBB',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: zksyncTokens.zk.address,
    token1Address: zksyncTokens.usdc.address,
  },
  {
    gid: 450,
    pairName: 'ZK-USDC.e',
    address: '0x2c39aB1C4e5376F03A7FF5947e842719Fb267f6e',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: zksyncTokens.zk.address,
    token1Address: zksyncTokens.usdc.address,
  },
  {
    gid: 451,
    pairName: 'ZK-USDC',
    address: '0x1fE7E46AA40Ea3b051348A58eE3c9135C1F9AEb3',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: zksyncTokens.zk.address,
    token1Address: zksyncTokens.usdcNative.address,
  },
  {
    gid: 452,
    pairName: 'ZK-USDC',
    address: '0xBc30878378FCb442E6dE822329d87714AFe43557',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: zksyncTokens.zk.address,
    token1Address: zksyncTokens.usdcNative.address,
  },
  {
    gid: 453,
    pairName: 'LISTA-BNB',
    address: '0x976241c208CC07505a620d768ee7251d47AcB2a0',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: bscTokens.wbnb.address,
    token1Address: bscTokens.lista.address,
  },
  {
    gid: 454,
    pairName: 'RF-ETH',
    address: '0x92B876878666214EAA19b1c3C24Ebd7D0F843F78',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: zksyncTokens.rf.address,
    token1Address: zksyncTokens.weth.address,
  },
  {
    gid: 455,
    pairName: 'LISTA-USDT',
    address: '0x2C533e2C2B4fd1172b5A0a0178805Be1526a15a7',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: bscTokens.lista.address,
    token1Address: bscTokens.usdt.address,
  },
  {
    gid: 456,
    pairName: 'SolvBTC.ena-SolvBTC',
    address: '0x04a35D7920f2f2fF9fAA447fF8cFaD47fc7cED2b',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.solvBTC.address,
    token1Address: arbitrumTokens.solvBTCena.address,
  },
  {
    gid: 457,
    pairName: 'BLB-ETH',
    address: '0x6FD5029Ba47BA8dBee130DA71e2546b5c96b4B12',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: ethereumTokens.blb.address,
    token1Address: ethereumTokens.weth.address,
  },
  {
    gid: 458,
    pairName: 'SolvBTC.ena-SolvBTC',
    address: '0x7C6CC4D67C920e4b86d46EA125f69b410afdaF61',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.solvbtc.address,
    token1Address: bscTokens.solvBTCena.address,
  },
  {
    gid: 459,
    pairName: 'ZRO-ETH',
    address: '0x152703894d31e54437d01b03BDD21c03583E9709',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: arbitrumTokens.zro.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 460,
    pairName: 'ZRO-ETH',
    address: '0xbee435c7Ff6Cb814124281C4C2532476A094Ac87',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: baseTokens.zro.address,
    token1Address: baseTokens.weth.address,
  },
  {
    gid: 461,
    pairName: 'ZRO-BNB',
    address: '0xC241e8fb314bc3799BE2Db94dCE4d070c29b425d',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: bscTokens.zro.address,
    token1Address: bscTokens.bnb.address,
  },
  {
    gid: 462,
    pairName: 'ZRO-ETH',
    address: '0x284651847C4B630D17a69A10af00871c6FBAcEEe',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: ethereumTokens.zro.address,
    token1Address: ethereumTokens.weth.address,
  },
  {
    gid: 463,
    pairName: 'mstETH-wstETH',
    address: '0x492fbedb478355DE0454e521fa2d8339F356e9B5',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.wstETH.address,
    token1Address: arbitrumTokens.mstETH.address,
  },
  {
    gid: 464,
    pairName: 'mstETH-wstETH',
    address: '0x0eC7a280709a03877E69D1b80C115F8e8D3abC02',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: bscTokens.weEth.address,
    token1Address: bscTokens.eth.address,
  },
  {
    gid: 465,
    pairName: 'SQD-WETH',
    address: '0xc24B560c7f8a1A50c2336cd8917Af61B5E14984F',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.HIGH,
    token0Address: arbitrumTokens.sqd.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 466,
    pairName: 'ZK-WETH',
    address: '0xb69c27573aED955e40905ac6Dba431aaeB88Feab',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.ALM,
    token0Address: zksyncTokens.zk.address,
    token1Address: bscTokens.eth.address,
  },
  {
    gid: 467,
    pairName: 'WETH-ZK',
    address: '0x8b55A31110d716F50009e02C337A16cA4EA9b675',
    chainId: ChainId.ZKSYNC,
    type: GaugeType.ALM,
    token0Address: bscTokens.eth.address,
    token1Address: zksyncTokens.zk.address,
  },
  {
    gid: 468,
    pairName: 'DEXE-BOXY',
    address: '0xe2cB548594fb8FbcF04B8EfddFB2EFaB1bA5A6e7',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: bscTokens.dexeTkn.address,
    token1Address: bscTokens.boxy.address,
  },
  {
    gid: 469,
    pairName: 'ETH-BRETT',
    address: '0x765bf105ed38D2Ee7801210B4Bb2B8b7D9B3A17f',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.brett.address,
  },
  {
    gid: 470,
    pairName: 'ETH-USDT',
    address: '0x526d54cD4FAc2e6B2ddCb6bC98b9292603061f85',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.usdt.address,
  },
  {
    gid: 471,
    pairName: 'ETH-DEGEN',
    address: '0x54D281c7cc029a9Dd71F9ACb7487dd95B1EecF5a',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.degen.address,
  },
  {
    gid: 472,
    pairName: 'ETH-AERO',
    address: '0x20CB8f872ae894F7c9e32e621C186e5AFCe82Fd0',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.aero.address,
  },
  {
    gid: 473,
    pairName: 'WBTC-USDT',
    address: '0xeAF03f385C642b02B5C563640BBA7c4FBf96c27D',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.usdt.address,
  },
  {
    gid: 474,
    pairName: 'WBTC-USDC.e',
    address: '0x06607df154875f0507C33418707F366bD47A62Fa',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.usdce.address,
  },
  {
    gid: 475,
    pairName: 'wstETH-USDC.e',
    address: '0xDC0CBC0e581730F0f9c110181582873eb4e116CB',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.wstETH.address,
    token1Address: arbitrumTokens.usdce.address,
  },
  {
    gid: 476,
    pairName: 'ZRO-ETH',
    address: '0xf0346b437Fe43e64Fa799C2BD3cf5Db1F7e9327C',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.zro.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 477,
    pairName: 'PENDLE-ETH',
    address: '0x24D76366bb47AbB4abc72022dC5Cb18DD85Ea259',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.pendle.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 478,
    pairName: 'USDe-USDT',
    address: '0xfA4971DC5aD81b4FccAFFAD0a584d13192B7D2BA',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usde.address,
    token1Address: arbitrumTokens.usdt.address,
  },
  {
    gid: 479,
    pairName: 'TAP-ETH',
    address: '0x89F5A7B6FF864F25f7Eb5a13309a83569F07FDbe',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: arbitrumTokens.tap.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 480,
    pairName: 'ETH+-ETH',
    address: '0xF24c54520F5a9AEa308AbEFEFF97bCcCAaef43db',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.ethPlus.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 481,
    pairName: 'ETH-GRT',
    address: '0x7cFa96FEc24C3Ea23dE73F9C91f612319847e3D0',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.grt.address,
  },
  {
    gid: 482,
    pairName: 'weETH-ETH',
    address: '0x302976A386fbb375033bE3Ac1E4112F76cf42ef7',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.weeth.address,
    token1Address: baseTokens.weth.address,
  },
  {
    gid: 483,
    pairName: 'WSI-USDT',
    address: '0x1c25ae16e6DE5760aa92eEBF2857EfF2796539d1',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: bscTokens.wsi.address,
    token1Address: bscTokens.usdt.address,
  },
  {
    gid: 484,
    pairName: 'wstETH-mstETH',
    address: '0xb9F9C2c31C3d00DD0EB987c818b4078F786d6F6E',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.wstETH.address,
    token1Address: arbitrumTokens.mstETH.address,
  },
  {
    gid: 485,
    pairName: 'MIGGLES-ETH',
    address: '0x3C288a41C135fb0Bae3F95b6A37B5e3e89f3FD95',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.miggles.address,
  },
  {
    gid: 486,
    pairName: 'deUSD-USDC',
    address: '0x689D028f13033d2E2A7c87806bEe05F45d859B52',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: ethereumTokens.deusd.address,
    token1Address: ethereumTokens.usdc.address,
  },
  {
    gid: 487,
    pairName: 'deUSD-USDC',
    address: '0x856B93E22127E6B76B79573bf95a93b4c88cb745',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: ethereumTokens.deusd.address,
    token1Address: ethereumTokens.usdt.address,
  },
  {
    gid: 488,
    pairName: 'USDT-DAI',
    address: '0x9fd6EBb5be86447616500c9Fc656F1247CBBd170',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.dai.address,
    token1Address: arbitrumTokens.usdt.address,
  },
  {
    gid: 489,
    pairName: 'USDT-USDC',
    address: '0x5f07bb9fEE6062e9D09A52E6d587c64bAD6bA706',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.usdt.address,
  },
  {
    gid: 490,
    pairName: 'USDe-USDT',
    address: '0x027039bd4eA6Ac7f80fEf4be2d0C6B74bfA932F9',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: ethereumTokens.usde.address,
    token1Address: ethereumTokens.usdt.address,
  },
  {
    gid: 491,
    pairName: 'SolvBTC-WBTC',
    address: '0xcFBcc53E96908Cc3719f2807457Db0b89870c4de',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.wbtc.address,
    token1Address: arbitrumTokens.solvbtc.address,
  },
  {
    gid: 492,
    pairName: 'SolvBTC.BBN-WBTC',
    address: '0xfb5076B8649022E9057FB6Eb7cbaC686CAcC2448',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.solvbtcbbn.address,
    token1Address: arbitrumTokens.solvbtc.address,
  },
  {
    gid: 493,
    pairName: 'SolvBTC.BBN-WBTC',
    address: '0x5a5ca75147550079411F6F543B729A4bEAb4dfEb',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.solvbtcbbn.address,
    token1Address: bscTokens.solvbtc.address,
  },
  {
    gid: 494,
    pairName: 'stBTC-BTCB',
    address: '0xfc18301B94a77D91015bb90D5249827c506846Ae',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.stbtc.address,
  },
  {
    gid: 495,
    pairName: 'USDe-USDC',
    address: '0x1CAF100CD74792D4be6C64621C2E21c7830868c4',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: arbitrumTokens.usde.address,
    token1Address: arbitrumTokens.usdc.address,
  },
  {
    gid: 496,
    pairName: 'USDC-AERO',
    address: '0x29a857629E58C531f351Bb7fbd86aa46Ef6aF0B7',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.aero.address,
  },
  {
    gid: 497,
    pairName: 'USDC-USD+',
    address: '0x167c9F0AF189DDF58f4B43683404a45096c23b67',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.usdPlus.address,
  },
  {
    gid: 498,
    pairName: 'USD+-USDbC',
    address: '0x504FbeB4Fe5E76A3E9747a88B4836F6dfa94185f',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.usdPlus.address,
    token1Address: baseTokens.usdbc.address,
  },
  {
    gid: 499,
    pairName: 'USDz-USDC',
    address: '0xebF0E4dA26A0d040C2343a9e072A0d0B8cF1E0b0',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.usdz.address,
    token1Address: baseTokens.usdc.address,
  },
  {
    gid: 500,
    pairName: 'USDz-ETH',
    address: '0x2cAB6049d0697a2bfc658f35443c5BA85eE88695',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: baseTokens.usdz.address,
    token1Address: baseTokens.weth.address,
  },
  {
    gid: 501,
    pairName: 'USD0-USDT',
    address: '0xBfab494D6311432Ed86Ee88779aCD7B4838920B7',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: ethereumTokens.usd0.address,
    token1Address: ethereumTokens.usdt.address,
  },
  {
    gid: 502,
    pairName: 'PENDLE-WETH',
    address: '0x042691F78269BA3d6325c945044239758aecC275',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: arbitrumTokens.pendle.address,
    token1Address: arbitrumTokens.weth.address,
  },
  {
    gid: 503,
    pairName: 'BRETT-ETH',
    address: '0x75CC10fdcEa4b7D13c115ABB08240ac9c9Be6f2f',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.brett.address,
  },
  {
    gid: 504,
    pairName: 'DEGEN-ETH',
    address: '0xFa98B3C0526C9bf96787e5797e87910f1175af0F',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.degen.address,
  },
  {
    gid: 505,
    pairName: 'MIGGLES-ETH',
    address: '0x4b381d7c8021fD23A979e64a582049C1D4E39F38',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    feeTier: FeeAmount.MEDIUM,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.miggles.address,
  },
  {
    gid: 506,
    pairName: 'swBTC-BTC',
    address: '0x365EA9E5Cec960390f35b6509548e84073168A8B',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: ethereumTokens.wbtc.address,
    token1Address: ethereumTokens.swBTC.address,
  },
  {
    gid: 507,
    pairName: 'wstETH-ETH',
    address: '0x6CB5392B9CA52d7a0E6940e82d29087361360eC3',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOWEST,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.wstETH.address,
  },
  {
    gid: 508,
    pairName: 'wstETH-BTCB',
    address: '0xcC1c80529B483A663d869C137a1Fd0cBD9855dC8',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.wstETH.address,
    token1Address: bscTokens.btcb.address,
  },
  {
    gid: 509,
    pairName: 'wstETH-BNB',
    address: '0xc9002d23CEBF00b40f7CDB5B1E63d96eebB50031',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.wstETH.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 510,
    pairName: 'kBTC-BTCB',
    address: '0x7b0240618CfE1cf4c576a905CA2557f287F97911',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    feeTier: FeeAmount.LOW,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.kbtc.address,
  },
  {
    gid: 511,
    pairName: 'mBTC-WBTC',
    address: '0xaB9dEBAED21270832cCdf54a7461A8E4B133B57A',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.StableSwap,
    tokenAddresses: [ethereumTokens.mBtc.address, ethereumTokens.wbtc.address],
  },
  {
    gid: 512,
    address: '0x9E143b6644C752d82F63a416c1f37c908f845BB4',
    pairName: 'WIF-ETH',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.wif.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 513,
    address: '0x62Fcb3C1794FB95BD8B1A97f6Ad5D8a7e4943a1e',
    pairName: 'ETH-BNB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.wbnb.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 514,
    address: '0x9F599F3D64a9D99eA21e68127Bb6CE99f893DA61',
    pairName: 'ETH-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 515,
    address: '0x247f51881d1E3aE0f759AFB801413a6C948Ef442',
    pairName: 'BTCB-USDT',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.usdt.address,
    token1Address: bscTokens.btcb.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 516,
    address: '0xB2DC4d7627501338B578985c214208eb32283086',
    pairName: 'ORDER-ETH',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.order.address,
    token1Address: ethereumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 517,
    address: '0xdfBEAF8f80955B9a2Bd1e3d7E1D68Fafd3aF130F',
    pairName: 'AERO-USDC',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.aero.address,
    token1Address: baseTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 518,
    address: '0x00efd5FCF7EB40D6aF892bDEC9e62bA3bA50e6d9',
    pairName: 'AERO-ETH',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.aero.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 519,
    address: '0xE9d76696f8A35e2E2520e3125875C3af23f1E69c',
    pairName: 'USDC-ETH',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 520,
    address: '0x4c98E9C2439C0d4621C62FeE2fED6D042fa8c570',
    pairName: 'ezETH-ETH',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.ezEth.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 521,
    address: '0xB5dad87dE341a1cDE301404442EB9c576a139298',
    pairName: 'OVN-USD+',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.ovn.address,
    token1Address: baseTokens.usdPlus.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 522,
    address: '0xB180Fa8919b737aa67C5578c23400F1eAC6C6D0e',
    pairName: 'MOG-ETH',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.mog.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 523,
    address: '0x216fDD5ec7dF0F3dc7D88A3c865ED93c0d8ad7be',
    pairName: 'USDC-eUSD',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.eUsd.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 524,
    address: '0xb6A5A6f2157F7894DF6925e13D42C23d18BdC37D',
    pairName: 'gUSDC-USDC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.gusdc.address,
    token1Address: arbitrumTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 525,
    address: '0xfE193D41Bf8EE691beC999bcE0981B6D06c89394',
    pairName: 'USD+-USDT',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.usdplus.address,
    token1Address: arbitrumTokens.usdt.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 526,
    address: '0x2163A4989B42748f28510CDDeC7a335ab656bB99',
    pairName: 'DOLA-USDC',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.dola.address,
    token1Address: baseTokens.usdc.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 527,
    address: '0xb039770090b68C2eeb6aa9709A918E5A1bbC914E',
    pairName: 'POL-USDC',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.pol.address,
    token1Address: polygonZkEvmTokens.usdce.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 528,
    address: '0x52De21560E97821a33fb7964F042f7575b78DDb5',
    pairName: 'POL-USDT',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.usdt.address,
    token1Address: polygonZkEvmTokens.pol.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 529,
    address: '0x6E6B4304195FD46c1Cec1F180e5225e7b4351ffF',
    pairName: 'POL-WETH',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.pol.address,
    token1Address: polygonZkEvmTokens.weth.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 530,
    address: '0xDCa5413A86fd039B79B4969a803522164336c323',
    pairName: 'POL-MATIC',
    chainId: ChainId.POLYGON_ZKEVM,
    type: GaugeType.V3,
    token0Address: polygonZkEvmTokens.pol.address,
    token1Address: polygonZkEvmTokens.matic.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 531,
    address: '0x24853895C8864135E77f3b13CF735966f7636f39',
    pairName: 'uniBTC-BTCB',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.uniBtc.address,
    token1Address: bscTokens.btcb.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 532,
    address: '0xaA4DABbF68AC17eF6dABFd11a7035b913C5F2de3',
    pairName: 'mBTC-BTCB',
    chainId: ChainId.BSC,
    type: GaugeType.StableSwap,
    tokenAddresses: [bscTokens.mBtc.address, bscTokens.btcb.address],
  },
  {
    gid: 533,
    address: '0x893c6d8b83726df868C1426f71c46Ba479ba3cC8',
    pairName: 'mBTC-WBTC',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.StableSwap,
    tokenAddresses: [arbitrumTokens.mBtc.address, arbitrumTokens.wbtc.address],
  },
  {
    gid: 534,
    address: '0x87Fe0e807E3E92A7558303FB51F201dcBfb07cC1',
    pairName: 'SEED-ETH',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.seed.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 535,
    pairName: 'WBNB-CKP',
    address: '0xeB184FD678854dE991FFF9a715A42eB933b9170E',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.ckp.address,
    token1Address: bscTokens.wbnb.address,
  },
  {
    gid: 536,
    pairName: 'WBNB-MGP',
    address: '0x660Ee2c04De3e3B74E7C5a9463fFbDE94b909eAe',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.wbnb.address,
    token1Address: bscTokens.mgp.address,
  },
  {
    gid: 537,
    pairName: 'cbBTC-WBTC',
    address: '0x3E253Ae716eBf5047d6deeE8f2607121ceA08482',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wbtc.address,
    token1Address: ethereumTokens.cbBTC.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 538,
    pairName: 'cbBTC-ETH',
    address: '0xF84AAd7ab687b1BB1FBB190344240b60B39e5b79',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.weth.address,
    token1Address: ethereumTokens.cbBTC.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 539,
    pairName: 'cbBTC-USDC',
    address: '0x5fbbC3380DD9f7253Aa10838DD641dAc8242F817',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.usdc.address,
    token1Address: ethereumTokens.cbBTC.address,
    feeTier: FeeAmount.LOW,
  },
  {
    gid: 540,
    pairName: 'cbBTC-ETH',
    address: '0xC211e1f853A898Bd1302385CCdE55f33a8C4B3f3',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.cbBTC.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 541,
    pairName: 'cbBTC-USDC',
    address: '0xb94b22332ABf5f89877A14Cc88f2aBC48c34B3Df',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.cbBTC.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 542,
    pairName: 'cbBTC-USDbC',
    address: '0x55B40f8911402568414ec6e7e0d2D87c3D19e65C',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.cbBTC.address,
    token1Address: baseTokens.usdbc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 543,
    pairName: 'LAVA-ETH',
    address: '0xB593f062D5F235862643fF0DBAb743d7316B5b9F',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.lava.address,
    token1Address: baseTokens.weth.address,
    feeTier: FeeAmount.MEDIUM,
  },
  {
    gid: 544,
    pairName: 'EURC-USDC',
    address: '0x1Ca42C7219F0cB1B67927e26502320cB98F725bd',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.eurc.address,
    token1Address: baseTokens.usdc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 545,
    pairName: 'EURC-ETH',
    address: '0xb57BdfDb5FE848f999B45A868131a50b2EBE1755',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.eurc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 546,
    pairName: 'EGP-WETH',
    address: '0x02c12Fd05Cb0d26579789AbCFdc58B07dB755236',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.egp.address,
    token1Address: arbitrumTokens.weth.address,
    feeTier: FeeAmount.HIGH,
  },
  {
    gid: 547,
    pairName: 'MGP-WETH YieldIQ',
    address: '0x5100245A8649586a00d3FE94c6B1b29B75537258',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.weth.address,
    token1Address: arbitrumTokens.mgp.address,
  },
  {
    gid: 548,
    pairName: 'wstETH-mstETH YieldIQ',
    address: '0x0A26ef06C183D51260Ca7434ddb510a6cd711c85',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.ALM,
    token0Address: arbitrumTokens.wstETH.address,
    token1Address: arbitrumTokens.mstETH.address,
  },
  {
    gid: 549,
    pairName: 'ETH-wstETH YieldIQ',
    address: '0x04dE953421b8c6B627D8787dDEeE433DE3411b71',
    chainId: ChainId.BSC,
    type: GaugeType.ALM,
    token0Address: bscTokens.eth.address,
    token1Address: bscTokens.wstETH.address,
  },
  {
    gid: 550,
    pairName: 'mBTC-WBTC',
    address: '0x0eEEE1eBa23fe157dBbD17ae2A1Bf9cd53EF7bDd',
    chainId: ChainId.ETHEREUM,
    type: GaugeType.V3,
    token0Address: ethereumTokens.wbtc.address,
    token1Address: ethereumTokens.mBtc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 551,
    pairName: 'mBTC-BTCB',
    address: '0x8ab639FdFdEafC175D29D3B6f49eA7745808A061',
    chainId: ChainId.BSC,
    type: GaugeType.V3,
    token0Address: bscTokens.btcb.address,
    token1Address: bscTokens.mBtc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 552,
    pairName: 'mBTC-WBTC',
    address: '0x81C71A616E67924a74Fea074A531e7c4F618fE56',
    chainId: ChainId.ARBITRUM_ONE,
    type: GaugeType.V3,
    token0Address: arbitrumTokens.mBtc.address,
    token1Address: arbitrumTokens.wbtc.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 553,
    pairName: 'BRETT-ETH',
    address: '0xa46179462f33b2c85134e5ec9Cac87073742560a',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.brett.address,
    feeTier: FeeAmount.LOWEST,
  },

  {
    gid: 554,
    pairName: 'DEGEN-ETH',
    address: '0xe80fd412AA92E0Ee8B86b419107616E05c00b77a',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.weth.address,
    token1Address: baseTokens.degen.address,
    feeTier: FeeAmount.LOWEST,
  },
  {
    gid: 555,
    pairName: 'AERO-USDC',
    address: '0xEa34EA0D44b035ce1B7D87D47a77E3B251013b06',
    chainId: ChainId.BASE,
    type: GaugeType.V3,
    token0Address: baseTokens.usdc.address,
    token1Address: baseTokens.aero.address,
    feeTier: FeeAmount.LOWEST,
  },
]
